...
describe('Agent Chat audio integration boundary', () => {
  it('uses the migrated IRIS audio controller rather than the legacy Ollama speech IPC path', () => {
    ...
    const settings = readFileSync(resolve(process.cwd(), 'src/platform-context/useAgentSettings.ts'), 'utf8')
    ...
    expect(settings).toContain('export function useAgentSettings')
