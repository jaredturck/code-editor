...
vi.mock('../../backend/desktopBridge/services/fileBrowserService', () => ({
...
vi.mock('../../backend/desktopBridge/services/fileIndexSourceService', () => ({
...
const clip = vi.hoisted(() => ({
  ...
  embedPrepared: vi.fn(async (images: Array<{ data: Uint8Array }>) =>
    images.map((image) => (image.data[0] === 1 ? [1, 0, ...Array(510).fill(0)] : [0, 1, ...Array(510).fill(0)])),
  ),
  embedText: vi.fn(async (text: string) =>
    text.toLowerCase().includes('cat') ? [1, 0, ...Array(510).fill(0)] : [0, 1, ...Array(510).fill(0)],
  ),
}))

vi.mock('../../backend/desktopBridge/services/fileClipService', () => ({
...
vi.mock('../../backend/desktopBridge/services/fileDocumentService', () => ({
...
vi.mock('../../backend/desktopBridge/services/filePdfService', () => ({
...
vi.mock('../../backend/desktopBridge/services/fileExtractionPool', () => ({
...
vi.mock('../../backend/desktopBridge/services/fileImageProcessingPool', () => ({
...
vi.mock('../../backend/desktopBridge/services/fileImageQueueBudget', () => ({
...
vi.mock('../../backend/desktopBridge/services/fileVideoService', () => ({
...
vi.mock('../../backend/desktopBridge/services/fileConceptService', () => ({
...
const storage = vi.hoisted(() => ({
  ...
  semantics: new Map<string, { fileId: string; metadata: Record<string, unknown>; embedding: number[] }>(),
...
vi.mock('../../backend/desktopBridge/storage/encryptedDatabase', () => ({
  ...
  readEncryptedFilesystemNodePage: vi.fn(
    async (options: {
      ...
      if (options.orderBySize) {
        nodes.sort((left, right) => right.sizeBytes - left.sizeBytes || left.id.localeCompare(right.id))
      } else {
  ...
  readEncryptedFileConceptMemberships: vi.fn(async (conceptIds: string[], limitPerConcept: number) => {
    const result = []
...
async function waitForIndex(
  service: typeof import('../../backend/desktopBridge/services/fileSemanticService'),
  root: string,
) {
...
describe('file semantic service', () => {
  it('classifies the tree once and embeds compact text representations with MiniLM', async () => {
    ...
    const service = await import('../../backend/desktopBridge/services/fileSemanticService')
    ...
    const nodesByName = new Map([...storage.nodes.values()].map((node) => [node.metadata.name, node]))
    ...
    expect([...storage.semantics.values()].map((record) => record.metadata.semanticType).sort()).toEqual([
      'image',
      'text',
    ])
    ...
    const calculatorResults = await service.searchFileSemanticIndex('calculator app in Python', 5, 'text')
  ...
  it('runs documents and PDFs as distinct content-based indexing stages', async () => {
    ...
    const semanticsByName = new Map(
      [...storage.semantics.values()].map((record) => [storage.nodes.get(record.fileId)?.metadata.name, record]),
    )
  ...
  it('normalizes MiniLM and CLIP score distributions before mixed ranking', async () => {
    ...
    for (const semantic of storage.semantics.values()) {
      ...
      semantic.embedding = [score, Math.sqrt(Math.max(0, 1 - score ** 2)), ...Array(Math.max(0, length - 2)).fill(0)]
    }
  ...
  it('stores independent video-frame embeddings and returns the best matching timestamp', async () => {
    ...
    clip.embedText.mockImplementation(async (text: string) =>
      text.toLowerCase().includes('lamborghini') ? [0, 0, 1, ...Array(509).fill(0)] : [0, 1, 0, ...Array(509).fill(0)],
    )
    const service = await import('../../backend/desktopBridge/services/fileSemanticService')
  ...
  it('skips an unreadable image without aborting the remaining image index', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(`Skipping unreadable image: ${badImagePath}`, expect.any(Error))
  })
  ...
  it('samples the beginning, middle, and end while keeping the content representation compact', async () => {
    ...
    const tailPadding = 'b'.repeat(1000 - prefix.length - middlePadding.length - middleMarker.length - endMarker.length)
    await fs.writeFile(
  ...
  it('reduces the calibration batch when Ollama drops an oversized socket', async () => {
    ...
    for (let index = 0; index < 16; index += 1) {
      await fs.writeFile(path.join(root, `sample-${index}.txt`), `sample ${index} ${'content '.repeat(80)}`)
    }
    const batchSizes: number[] = []
    vi.stubGlobal(
      'fetch',
  ...
  it('keeps CLIP preparation failures in semantic index status', async () => {
    ...
    await expect(service.installFileSemanticModels(root)).rejects.toThrow('IRIS could not prepare the CLIP image model')
  ...
  it('rescans without re-embedding unchanged files and reclassifies changed files', async () => {
    ...
    await service.rebuildFileSemanticIndex(root)
    await waitForIndex(service, root)
    const initialEmbedCalls = fetchMock.mock.calls.filter(([url]) => String(url).endsWith('/api/embed')).length

    await service.rescanFileSemanticIndex(root)
    await waitForIndex(service, root)
    expect(fetchMock.mock.calls.filter(([url]) => String(url).endsWith('/api/embed')).length).toBe(initialEmbedCalls)
    ...
    expect(
      [...storage.semantics.values()].some((record) => record.fileId === nodesByName.get('calculator.py')?.id),
    ).toBe(false)
  })
})
