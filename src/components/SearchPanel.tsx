...
import { powerFd, powerRipgrep } from '../platform/desktopBridge'
...
function SearchPanel({ rootPath, onOpenFile }: { rootPath: string | null; onOpenFile: (file_path: string) => void }) {
  ...
  const [search_mode, set_search_mode] = useState<'text' | 'files'>('text')
          ...
          <div className="min-w-0 flex-1 space-y-1.5">
            <div className="flex h-8 items-center rounded border border-[var(--input-border)] bg-[var(--input-bg)] px-2 focus-within:border-sky-500">
              <input
                ...
                placeholder={search_mode === 'files' ? 'Search file names' : 'Search'}
                type="text"
                value={query}
              />
        ...
        {!loading && results.length > 0 && (
          <>
            ...
            {results.map((result, index) => (
              <button
                className="block w-full rounded px-1.5 py-1.5 text-left hover:bg-[var(--hover)]"
              ...
              >
                ...
                {result.content && (
                  <div className="mt-0.5 truncate font-mono text-[10px] text-[var(--muted)]">{result.content}</div>
                )}
              </button>
            ))}
          </>
