...
function placeNoteAtGroupStart(notes: StoredNote[], note: StoredNote): StoredNote[] {
  ...
  return reindexNoteOrder(
    note.pinned ? [note, ...pinned, ...regular] : [...pinned, note, ...regular],
  );
}

function insertDuplicateAfterSource(
  notes: StoredNote[],
  sourceId: number,
  duplicate: StoredNote,
): StoredNote[] {
...
export function useNotesPanel() {
  ...
  const updateNote = (id: StoredNote['id'], updates: NoteUpdates) => {
    ...
    setNotes((previous) =>
      previous.map((note) => (note.id === id ? { ...note, ...updates, updatedAt } : note)),
    );
  };
      ...
      messages: [
        {
          role: 'system',
          content:
            'Summarize and improve the following note concisely. Preserve important facts and action items.',
        },
