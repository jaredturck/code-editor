...
export function normalizeApprovalDecision(value: unknown): string {
  ...
  if (['continue', 'continue_once', 'continue-once', 'once', 'retry'].includes(token))
    return 'continue';
  if (['extend', 'extend_budget', 'extend-budget', 'increase_budget'].includes(token))
    return 'extend';
  if (
    [
      'unlimited',
...
export function isApprovalDecisionApproved(decision: unknown): boolean {
  return ['approve', 'continue', 'extend', 'unlimited'].includes(
    normalizeApprovalDecision(decision),
  );
}
...
export function normalizeApprovalOptions(
  request: Partial<ApprovalRequest> | null | undefined,
): ApprovalOption[] {
  ...
  const normalized = input
    .map((option) => {
      ...
      const id = normalizeApprovalDecision(
        source?.id || source?.value || source?.decision || source?.label,
      );
      if (!id) return null;
      return {
        id,
