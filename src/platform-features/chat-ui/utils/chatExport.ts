...
export function downloadTextFile(fileName: unknown, text: unknown, mimeType = 'text/plain;charset=utf-8'): boolean {
...
export function formatChatMarkdown(messages: ChatMessage[], meta: UnknownRecord = {}): string {
  ...
  for (const message of Array.isArray(messages) ? messages : []) {
    ...
    const who = message.role === 'user' ? 'You' : message.role === 'assistant' ? 'IRIS' : message.role || 'note'
