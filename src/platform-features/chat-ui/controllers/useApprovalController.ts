...
export interface ApprovalController {
  ...
  resolveApprovalRequest(requestId: string, decision?: string, options?: ResolveApprovalOptions): Promise<void>
...
export function useApprovalController(setStatus: React.Dispatch<React.SetStateAction<string>>): ApprovalController {
  ...
  const resolveApprovalRequest = async (
    ...
    if (requestType === 'limit') {
      ...
      else if (normalizedDecision === 'continue') setStatus('Continuing task with one-time limit override.')
      else if (normalizedDecision === 'extend') setStatus('Limit budget extended for this run.')
      else if (normalizedDecision === 'unlimited') setStatus('Unlimited session mode enabled for this run.')
    ...
    setStatus(approved ? 'Approval granted for this session run.' : 'Approval denied for this session run.')
  }

  // Resolves question request from the available configuration and runtime context.
  const resolveQuestionRequest = (requestId: string, answer: string, options: ResolveApprovalOptions = {}): void => {
    const resolver = approvalResolversRef.current.get(requestId)
    approvalResolversRef.current.delete(requestId)
    const timeoutId = approvalTimeoutsRef.current.get(requestId)
    if (timeoutId) {
