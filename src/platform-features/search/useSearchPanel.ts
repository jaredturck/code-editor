...
export type SearchPhaseStatus = 'idle' | 'running' | 'reading' | 'generating' | 'complete' | 'cancelled' | 'failed'
...
function normalizePhase(value: unknown): StoredSearchPhase {
  ...
  return {
    ...
    result: phase.result && typeof phase.result === 'object' ? (phase.result as WebResearchResult) : null,
...
function normalizeSession(value: Record<string, unknown>): StoredWebSearchSession {
  ...
  return {
    ...
    followUps: Array.isArray(value.followUps) ? (value.followUps as StoredSearchFollowUp[]).slice(0, 100) : [],
...
export function useSearchPanel(): SearchPanelState {
  ...
  const queueToken = useCallback((sessionId: string, kind: keyof StreamedAnswers, token: string) => {
  ...
  const search = useCallback(async () => {
    ...
    try {
      ...
      if (session.id && isAbortError(searchError)) {
        ...
        const message = searchError instanceof Error ? searchError.message : String(searchError || 'Search failed')
        setError(message)
        if (session.id) {
          session = {
            ...session,
  ...
  const deleteHistory = useCallback(
    async (id: string) => {
      ...
      if (activeOperationRef.current?.sessionId === id) activeOperationRef.current.controller.abort()
