...
import { serializeSkillToMarkdown, parseSkillMarkdown, blankSkillMarkdown } from '@/platform/skillMarkdown'
...
export function useSkillsPanel() {
  ...
  const refreshSkills = useCallback(
    async (profileName: string) => {
      ...
      try {
        ...
        setMarkdown('')
        setHasDraft(false)
        setStatus((error as { message?: string } | null | undefined)?.message || 'Failed to load skills.')
      } finally {
        setLoading(false)
      }
    },
  ...
  const saveSkill = async () => {
    ...
    try {
      ...
      setStatus((error as { message?: string } | null | undefined)?.message || 'Failed to save skill.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const removeSkill = async (id: string) => {
    ...
    try {
      ...
      setStatus((error as { message?: string } | null | undefined)?.message || 'Failed to delete skill.')
    } finally {
      setSaving(false)
    }
  }
