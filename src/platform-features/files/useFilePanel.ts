...
export function useFilePanel() {
  ...
  const [indexAction, setIndexAction] = useState<'preflighting' | 'installing' | 'starting' | 'refreshing' | ''>('')
  ...
  const visibleEntries = useMemo(() => {
    ...
    return [...entries].sort((left, right) => compareEntries(left, right, sortField, sortDirection, searchMode))
  ...
  const selectFile = async (file: FileDisplayEntry) => {
    ...
    try {
      if (file.isImage) {
        ...
        const nextContent = result.isBinary ? '[Binary file preview is unavailable.]' : result.content
        setSelectedFile({
          ...file,
          path: result.path,
  ...
  const copyFileContents = async (file: FileDisplayEntry) => {
    ...
    try {
      ...
      setBrowserError((error as { message?: string }).message || 'Failed to copy the file contents')
    }
  }
  ...
  const openFileLocation = async (file: FileDisplayEntry) => {
    ...
    try {
      ...
      setBrowserError((error as { message?: string }).message || 'Failed to open the file location')
    }
  }
  ...
  const refreshDirectory = (permissionGranted = false) => loadDirectory(currentPath, permissionGranted)
  ...
  const refreshIndexSources = useCallback(async () => {
    ...
    try {
      ...
      const buildingSources =
        activeStatus?.indexStatus === 'building' && Array.isArray(activeStatus.sources) ? activeStatus.sources : []
      setIndexSources(buildingSources.length ? buildingSources : state.sources)
      setSelectedSourceIds(
        buildingSources.length ? buildingSources.map((source) => source.id) : state.selectedSourceIds,
      )
      setLocationsLocked(state.locked || activeStatus?.indexStatus === 'building')
      return state
    } catch (error) {
      setLocationsError((error as { message?: string }).message || 'Failed to discover index locations')
      return null
    } finally {
  ...
  const startInitialScan = async () => {
    ...
    setIndexError('')
    setIndexAction(
      semanticStatus?.embeddingModelInstalled && semanticStatus?.imageModelInstalled ? 'starting' : 'installing',
    )
    try {
