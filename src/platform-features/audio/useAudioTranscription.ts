...
export function useAudioTranscription<T>({
  activeTarget,
  onTranscript,
}: UseAudioTranscriptionOptions<T>) {
    ...
    async (target: T, mimeType: string, operation: number) => {
      ...
      try {
        ...
        if (mountedRef.current) {
          ...
          setStatusText(
            binding.cloud
              ? `Transcribing with ${binding.label}…`
              : 'Transcribing locally with Granite Speech…',
          );
        }
        ...
        if (mountedRef.current) {
          ...
          setError('');
          setStatusText(
            result.fallbackUsed ? 'Cloud transcription failed; local Granite completed it.' : '',
          );
          setPhase('idle');
        }
      } catch (value) {
  ...
  const prepareRecording = useCallback(
    async (target: T, bridgeSynchronized = false) => {
      ...
      setPhase('checking');
      setStatusText(
        binding.cloud ? `Preparing ${binding.label} transcription…` : 'Checking Ollama…',
      );
