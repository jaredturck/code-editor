...
export function resolveAudioTranscriptionBinding(
  settings: Pick<
    OrbSettings,
    'audio_provider' | 'audio_model' | 'audio_key_id' | 'audio_local_fallback'
  >,
): AudioTranscriptionBinding {
  const provider = String(settings.audio_provider || LOCAL_TRANSCRIPTION_PROVIDER).toLowerCase();
  const definition =
    AUDIO_PROVIDER_DEFINITIONS.find((entry) => entry.id === provider) ||
...
export function audioModelsForProvider(provider: unknown): readonly string[] {
  return (
    AUDIO_PROVIDER_DEFINITIONS.find((entry) => entry.id === String(provider || '').toLowerCase())
      ?.models || []
  );
}
