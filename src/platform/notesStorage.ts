...
function normalizeNote(value: unknown, fallbackId: number, fallbackSortOrder = 0): StoredNote {
  ...
  const normalizedColor: NoteColor = NOTE_COLOR_SET.has(rawColor)
  ...
  const rawCategory = String(note.category || header.category || 'general')
    .toLowerCase()
    .trim();
  const category: NoteCategory = NOTE_CATEGORIES.has(rawCategory)
...
function normalizeNotes(notes: unknown): StoredNote[] {
  if (!Array.isArray(notes)) {
    return normalizeNoteOrder(
      DEFAULT_NOTES.map((note, index) => normalizeNote(note, Number(note.id), index)),
    );
  }
  if (!notes.length) return [];

  return normalizeNoteOrder(
    notes.slice(0, 200).map((note, index) => normalizeNote(note, Date.now() + index, index)),
  );
}
...
export function recordUserPreferenceNote(correction: string): void {
  addNote({
    ...
    content: [
      'CATEGORY: user-preference',
      `SUMMARY: ${correction.slice(0, 200)}`,
      '',
      correction,
    ].join('\n'),
    color: 'green',
