...
export function trackUsageSample(
  ...
  const providerPromptTokens =
    Number.isFinite(Number(usage?.promptTokens)) && Number(usage?.promptTokens) > 0
  ...
  if (!hasProviderUsage) {
    ...
    if (!promptTokens && !completionTokens && totalTokens > 0) {
      const estimatedPrompt = Math.max(
        0,
        Math.min(totalTokens, estimatePromptTokensFromMessages(messages)),
      );
    ...
    if (!promptTokens) {
      ...
      promptTokens =
        totalTokens > 0 ? Math.max(0, Math.min(totalTokens, estimatedPrompt)) : estimatedPrompt;
      usedEstimate = true;
    }
...
export function buildUsageSummary(tracker: UsageTracker | null | undefined) {
  ...
  const cacheHitRatio =
    promptInputTotal > 0 ? Math.round((cacheReadTokens / promptInputTotal) * 1000) / 1000 : 0;
  ...
  const nativeToolAdoption =
    nativeSteps + jsonSteps > 0
      ? Math.round((nativeSteps / (nativeSteps + jsonSteps)) * 1000) / 1000
      : 0;
  ...
  const contextUsedPct =
    contextWindow > 0 ? Math.min(100, Math.round((totalTokens / contextWindow) * 1000) / 10) : 0;
