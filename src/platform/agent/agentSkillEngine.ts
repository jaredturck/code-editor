...
export function normalizeSkill(raw: unknown, index: number): NormalizedSkill {
  ...
  return {
    ...
    modelVariants: isRecord(source.modelVariants) ? (source.modelVariants as SkillModelVariants) : {},
...
export function scoreSkill(
  ...
  const _haystack = [skill.title, skill.summary, skill.instructions.slice(0, 800), skill.triggers.join(' ')]
    .join(' ')
    .toLowerCase()
...
export function selectSkillsForPrompt(
  allSkills: unknown,
  { userInput, conversation, settings, role = '', offloadedIds = null, toolset = 'structured' }: SkillSelectionOptions,
): SkillSelectionResult {
  const _tokenBudget = Math.max(
    200,
    Math.min(16000, Number(settings?.skills_token_budget || _DEFAULT_SKILLS_TOKEN_BUDGET)),
  )
  const _maxActive = Math.max(1, Math.min(16, Number(settings?.skills_max_active || _DEFAULT_SKILLS_MAX_ACTIVE)))
  ...
  const _offloaded =
    offloadedIds instanceof Set ? offloadedIds : new Set(Array.isArray(offloadedIds) ? offloadedIds : [])
  ...
  const _resolveInstructions = (skill: NormalizedSkill): string =>
    toolset !== 'lean' && skill.modelVariants?.simple ? skill.modelVariants.simple : skill.instructions || ''

  const _recentText = String(userInput || conversation.slice(-1)[0]?.content || '').toLowerCase()
  const _queryText = [String(userInput || ''), ...conversation.slice(-6).map((m) => String(m?.content || ''))]
    .join(' ')
    .toLowerCase()
  ...
  const _ranked = skills
    ...
    .sort(
      (a, b) => b.score - a.score || b.skill.priority - a.skill.priority || a.skill.title.localeCompare(b.skill.title),
    )
  ...
  const _cardLimit =
    toolset === 'lean' ? Math.min(_nonGuardRanked.length, _MAX_LEAN_CARD_COUNT) : _STRUCTURED_CARD_COUNT
...
function _toolResultFailed(result: unknown): boolean {
  ...
  const status = String(result.status || result.state || '')
    .trim()
    .toLowerCase()
  return result.ok === false || Boolean(result.error) || ['error', 'failed', 'failure'].includes(status)
}
...
function _evaluateReflexCondition(condition: unknown, result: unknown): boolean {
  ...
  const match = raw.match(/^result\.([a-zA-Z_][a-zA-Z0-9_.]*)\s*(===|!==|>=|<=|>|<)\s*(-?\d+(?:\.\d+)?)$/)
...
export function checkReflexSkills(toolName: unknown, toolResult: unknown, allSkills: unknown): NormalizedSkill[] {
