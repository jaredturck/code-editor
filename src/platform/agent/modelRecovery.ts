...
} from '@/platform/providers/localModelCatalog';
...
export async function recommendRecoveryModel(
  ...
  const excludeSet = new Set(
    exclude.map((entry) => identity(entry.provider, entry.model, entry.keyId)),
  );
  const hardware = await systemStats().catch(() => null);
  const candidates = listAvailableCandidates(settings)
    .filter(
      (candidate) =>
        !excludeSet.has(identity(candidate.provider, candidate.model, candidate.keyId)),
    )
