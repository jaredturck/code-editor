...
function candidateSettings(
  ...
  return {
    ...
    ai_runtime_api_key: candidate.provider === 'local' ? '' : getKey(candidate.provider, candidate.keyId || '1'),
...
export async function runBoundedRoleTask(options: BoundedRoleTaskOptions): Promise<BoundedRoleTaskResult> {
  ...
  for (let index = 0; index < candidates.length && index < maxAttempts; index += 1) {
    ...
    try {
      ...
      if ((emittedText || emittedThinking) && index + 1 < Math.min(candidates.length, maxAttempts)) {
