...
function buildLocalOnlyModelPool(
  allModels: AgentModelEntry[],
  responder: ChatResponderChoice,
): AgentModelEntry[] {
...
export function getChatResponderChoices(
...
export function getDefaultChatResponder(
  settings: SettingsLike | null | undefined,
...
export function buildChatExecutionSettings<T extends SettingsLike>(
  ...
  const policy: AgentExecutionPolicy =
    responder.provider === 'local' ? 'local_only' : requestedPolicy || 'hybrid';
  ...
  return {
    ...
    ai_runtime_api_key:
      responder.provider === 'local' ? '' : getKey(responder.provider, responder.keyId || '1'),
