...
export function attachSessionFilesToMessages(messages, settings) {
  ...
  const hydrated = messages.map((message) => {
    ...
    return message?.role === 'user' && persisted.length
      ? toProviderContent(message, persisted)
      : message;
  });
  ...
  const targetIndex = hydrated.reduce(
  ...
  return hydrated.map((message, index) =>
    index === targetIndex ? toProviderContent(message, attachedFiles) : message,
  );
}
 ...
 */
export function buildSessionSafetySnapshot({
  safetyConfig,
  sessionStepBudget,
  approvalState,
  sessionLimitState,
...
export async function runAgentSession({
  ...
  const cloudUsageState = createCloudUsageState(
    requestedResponderSettings,
    Boolean(hybridExecution?.finalResponder),
  );
  if (hybridExecution) {
  ...
  if (settings?.agent_primary_locked !== true && isModelRoutingEnabled(settings)) {
    try {
      ...
      const picked = pickModelForComplexity(
        candidates,
        estimateTaskComplexity(userInput, conversation),
      );
      if (picked?.id) {
  ...
  let sessionBudgetMs =
    Math.max(1, Number(settings?.agent_session_minutes) || AGENT_SESSION_MINUTES_DEFAULT) * 60000;
...
${recommendation.reason}`,
      ...
      try {
        ...
        const localModels = Array.from(
          new Set([
            ...
            ...(Array.isArray(discovered.local) ? discovered.local : []),
          ]),
        );
        ...
        settings = {
          ...
          agent_models: [
            ...(Array.isArray(settings.agent_models) ? settings.agent_models : []),
            entry,
          ],
        };
        writeOrbSettings({
  ...
  const tryFailover = async (error, step) => {
    ...
    const switchLabel =
      policy.maxAttempts >= 12
        ? `switch ${failoverSwitches}`
        : `switch ${failoverSwitches}/${policy.maxAttempts}`;
    onEvent?.({
      type: 'notice',
      level: 'error',
  ...
  if (settings?.agent_multi_enabled === true) {
    try {
      ...
      if (modeInfo.mode !== 'full') {
        const offlineStr = modeInfo.offline.length
          ? ` (${modeInfo.offline.join(', ')} offline)`
          : '';
    ...
    if (connectedMembers.length) {
      const memberList = connectedMembers
        .map((m) => {
          const tagStr =
            Array.isArray(m.tags) && m.tags.length ? `, ${m.tags.slice(0, 3).join('/')}` : '';
  ...
  const planningActive =
    settings?.agent_planning_mode === true && settings?.agent_multi_enabled === true;
  ...
  if (
    ...
    try {
      ...
      const detail = (
        overwatchErr instanceof Error ? overwatchErr.message : String(overwatchErr || 'error')
      ).slice(0, 120);
      onEvent?.({
        type: 'notice',
  ...
  if (planningActive) {
    ...
    if (approvedPlan && Array.isArray(approvedPlan.parts) && approvedPlan.parts.length) {
      ...
      const planLines = parts
        .map(
          (p) =>
            `• [${p.owner}] ${p.summary}${p.dependsOn?.length ? ` (after ${p.dependsOn.join(', ')})` : ''}`,
        )
        .join('  ');
      planNote =
  ...
  const resolveSessionToolTimeoutMs = (toolName) =>
    resolveSessionToolTimeout(toolName, sessionLimitState);
  ...
  const applyLimitDecision = ({ decision, kind, toolName, context = {} }) => {
    ...
    if (normalizedDecision === 'unlimited') {
      ...
      if (webSearchState) {
        webSearchState.maxCalls = Math.max(
          Number(webSearchState.maxCalls || 0),
          SEARCH_WEB_UNLIMITED_CALL_BUDGET,
        );
      }
      ...
      const unlimitedRetryDelayMs =
        kind === 'rate_limit'
          ? Math.max(2000, Math.min(30000, Number(context?.retryAfterMs || 10000)))
          : 0;
      ...
      return {
        ...
        message:
          'Unlimited session mode enabled for this run (expanded search and timeout limits).',
      };
    }
    ...
    if (kind === 'rate_limit') {
      ...
      const retryDelayMs = Math.max(
        1500,
        Math.min(30000, suggestedDelayMs > 0 ? suggestedDelayMs : decisionDelayMs),
      );
    ...
    if (kind === 'search_budget') {
      ...
      if (webSearchState) {
        const currentBudget = Number.isFinite(Number(webSearchState.maxCalls))
          ...
          : SEARCH_WEB_DEFAULT_CALL_BUDGET;
        webSearchState.maxCalls = Math.min(
          SEARCH_WEB_UNLIMITED_CALL_BUDGET,
          currentBudget + increment,
        );
      }
  ...
  const continuousOverwatch =
    settings?.agent_multi_enabled === true &&
    settings?.agent_overwatch_continuous === true &&
    hasOverwatcher(settings);
  ...
  const detectDrift = () => {
    ...
    for (
      let i = stepHistory.length - 1;
      i >= 0 && stepHistory[i] && stepHistory[i].ok === false;
      i -= 1
    ) {
      trailingFailures += 1;
    }
    ...
    const repeatedTool =
      recent.length >= 3 && new Set(recent.map((s) => String(s?.tool || ''))).size === 1;
  ...
  const dispatchCloudRequest = async ({
    candidate,
    purpose,
    reason,
    messages,
  ...
  const requestCloudConsult = hybridExecution
    ? async ({ question, reason, context, step }) => {
        ...
        const contextText =
          typeof context === 'string' ? context : context ? JSON.stringify(context, null, 2) : '';
  ...
  const finalizeHybridReply = async (localDraft) => {
    ...
    if (!state || !canUseCloud(state, 'final')) {
      traceTool.raw({
        ...
        level: 'error',
        summary:
          'Cloud request budget exhausted before final synthesis. Returning the local result.',
      });
      return draft;
    }
  ...
  if (continuityContext.resumeIntent) {
    traceTool.thinking(
      'Resume intent detected — relevant prior session notes will be surfaced if any match.',
      { channel: 'steering' },
    );
  }
  ...
  const forwardSubAgentEvent = (evt) => {
    ...
    const roleLabel = memberId.includes('#')
      ? `${baseLabel} #${memberId.split('#')[1]}`
      : baseLabel;
  ...
  const unsubscribeSubAgents =
    settings?.agent_multi_enabled === true
      ? subscribeSubAgentEvents(forwardSubAgentEvent)
      : () => {};
  ...
  if (useStatefulLoop(settings)) {
    ...
    const statefulTools = buildJsonSchemaTools(
      Array.isArray(initialPayload.tools) ? initialPayload.tools : [],
    );
      ...
      if (limitIssue && onApprovalRequest) {
        ...
        if (applied.approved) {
          ...
          if (applied.allowRetry) {
            const retryDelayMs = Math.max(
              0,
              Number(applied.retryDelayMs || limitContext.retryAfterMs || 0),
            );
    ...
    while (true) {
      ...
      if (await checkDurationBudget(step)) {
        ...
        return finishStateful(
          limitStopReason ||
            `Stopped after working for the allotted time. Send another message to continue.`,
        );
      }
      ...
      if (!toolCalls.length) {
        ...
        if (finalText && !looksLikeControllerSchemaText(finalText)) {
          ...
          if (
            ...
            questionAwaits += 1;
            traceTool.thinking(
              'Asked the user a question — pausing for their answer (the run stays open).',
              { step },
            );
            let answer = '';
            let stopped = false;
      ...
      for (const call of toolCalls) {
        ...
        if (guardVerdict.blocked) {
          traceTool.thinking(
            `Tool guard: blocked a repeated ${toolName} call. ${guardVerdict.reason}`,
            { step },
          );
          stepHistory.push({
            step,
      ...
      if (escalateAfterTurn) {
        traceTool.thinking(
          'Tool guard: repeated blocked calls — finalizing with current findings.',
          { step },
        );
        break;
      }
    ...
    const reply = stoppedByUser
      ...
      : limitStopReason
        ...
        : await synthesizeFinalReply({
            ...
            (error) =>
              `I could not complete the full agent flow. ${error?.message || 'Unknown error.'}`,
          );
  ...
  for (let step = 1; ; step += 1) {
    ...
    if (
      contextRemaining < modelContextWindow * CONTEXT_BUDGET_WARN_RATIO &&
      stepHistory.length >= 3
    ) {
      traceTool.thinking(
      ...
      try {
        const historyDigest = stepHistory
          .map(
            (s, i) =>
              `Step ${i + 1} [${s.tool}] ${s.ok ? 'ok' : 'error'} — ${String(s.summary || '').slice(0, 200)}`,
          )
          .join('\n');
    ...
    const useNativeTools =
      supportsNativeTools(effProvider, effModel) && settings?.native_tools_enabled !== false;
    ...
    try {
      ...
      try {
        ...
        if (reflexSkills.length > 0) {
          const reflexText = reflexSkills
            .map((s) => `REFLEX [${s.title}]: ${s.instructions}`)
            .join('\n');
          traceTool.thinking(
            `Reflex skill(s) triggered by ${toolName}: ${reflexSkills.map((s) => s.title).join(', ')}`,
            { step },
  ...
  if (!fallbackReply) {
    fallbackReply = await synthesizeFinalReply({
      ...
      (error) => `I could not complete the full agent flow. ${error?.message || 'Unknown error.'}`,
    );
  ...
  const rewardStoreFallback = recordReward(
    rewardResultFallback,
    String(userInput || '').slice(0, 60),
  );
  onEvent?.({
    type: 'reward',
