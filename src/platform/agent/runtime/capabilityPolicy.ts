...
import { resolveCurrentRole } from '@/platform/orchestrationClient'
...
export function summarizeTree(node) {
  ...
  return {
    ...
    dirs,
    total: files + dirs,
    topEntries: Array.isArray(node.children) ? node.children.slice(0, 12).map((child) => child.name) : [],
  }
}
...
export function resolveSafetyConfig(settings, maxSteps) {
  const profile =
    String(settings?.agent_safety_profile || 'strict').toLowerCase() === 'balanced' ? 'balanced' : 'strict'

  const configuredMaxSteps = clampNumber(settings?.agent_max_steps ?? MAX_AGENT_STEPS, 1, AGENT_STEP_HARD_CAP)
  ...
  const requestedMaxSteps =
    maxSteps === null || maxSteps === undefined ? configuredMaxSteps : clampNumber(maxSteps, 1, AGENT_STEP_HARD_CAP)
  ...
  const agentTier = Number.isFinite(Number(agentTierRaw)) ? Math.max(0, Math.min(3, Number(agentTierRaw))) : defaultTier
...
export function inferToolNameFromAliasKey(aliasKey) {
  ...
  if (has('resource', 'resources', 'capability', 'capabilities', 'permission', 'permissions', 'tools', 'tooling')) {
  ...
  if (
    ...
    has('file', 'files', 'dir', 'dirs', 'directory', 'folder', 'folders', 'path', 'workspace', 'text', 'content')
  ) {
    return 'files.find'
  }
  ...
  if (has('write', 'save', 'create', 'append', 'edit', 'update') && has('file', 'files', 'path', 'document', 'text')) {
    return 'files.write'
  }
  ...
  if (has('screen', 'display', 'automation', 'vision') && has('capability', 'capabilities', 'check', 'status')) {
    return 'screen.capabilities'
  }

  if (has('launch', 'start', 'open') && has('app', 'application', 'program', 'command', 'script', 'url')) {
    return 'launch.run'
  }

  if (has('approval', 'approve', 'permission', 'permissions') && has('request', 'ask', 'prompt', 'confirm')) {
...
export function evaluateToolAccess(
  ...
  if (name === 'agent.review' && !isPeerReviewEnabled(settings)) {
    return {
      available: false,
      reason: 'Multi-model peer review is off. Enable the bridge and set agent_peer_review in Settings → Agents.',
  ...
  if (name === 'agent.overwatch' && !isOverwatchEnabled(settings)) {
    return {
      available: false,
      reason: 'No Overwatcher is configured. Assign a reasoning model to the Overwatcher role in Settings → Agents.',
  ...
  const permissionEnabled = {
    ...
    terminal_exec: settings?.permissions_terminal === true || sessionPermissions.terminal_exec === true,
  }
...
export function buildCapabilitySnapshot({
  ...
  const executionMode = provider === 'local' ? 'local-model-with-local-tools' : 'cloud-model-with-local-tools'
  ...
  const tools = TOOL_DEFINITIONS.filter((tool) => tool.name !== 'todo.update' && tool.name !== 'trace.log').map(
    (tool) => {
...
export function buildInsufficientAccessReply(stepHistory, capabilitySnapshot) {
  ...
  const reasons = Array.from(new Set(blocked.map((step) => String(step.error || '').trim()).filter(Boolean)))
...
export function buildExecutableProbeCommand(targetText) {
  ...
  const quotedCandidates = candidates.map((candidate) => escapeSingleQuotedShellArg(candidate)).join(' ')
...
export function fallbackForcedToolAction(userInput, capabilitySnapshot) {
  ...
  if (searchIntent && availableTools.has('files.find')) {
    ...
    const remainder = String(userInput || '')
      .replace(/^(?:please\s+)?(?:can\s+you\s+|could\s+you\s+)?(?:find|locate|search(?:\s+for)?)\s+/i, '')
      .trim()
  ...
  const listIntent =
    /\b(list|show)\b/.test(text) && /\b(file|files|folder|folders|directory|directories|workspace)\b/.test(text)
...
export async function inferForcedToolActionForRequest({ userInput, capabilitySnapshot, requestAI }) {
  if (!isImperativeActionRequest(userInput)) return null

  const advertisedTools = Array.isArray(capabilitySnapshot?.advertisedTools)
    ? capabilitySnapshot.advertisedTools
    : capabilitySnapshot?.availableTools
  const availableTools = Array.isArray(advertisedTools) ? advertisedTools.filter((name) => TOOL_BY_NAME[name]) : []

  if (!availableTools.length) {
    return null
  }

  const availableToolSpecs = TOOL_DEFINITIONS.filter((tool) => availableTools.includes(tool.name)).map((tool) => ({
    name: tool.name,
    module: tool.module,
    description: tool.description,
    args: tool.args,
...
export function inferFindQuery({ args, userInput, todos }) {
  ...
  const todoText = Array.isArray(todos) ? todos.map((todo) => String(todo?.text || '')).join(' ') : ''
