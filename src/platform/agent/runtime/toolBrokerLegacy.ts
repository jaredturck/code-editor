...
function isWebSiteAllowed(host, settings, approvalState) {
  ...
  if (
    approvalState?.webSiteSessionDomains instanceof Set &&
    approvalState.webSiteSessionDomains.has(host)
  )
    return true;
  const allow = Array.isArray(settings?.agent_web_allowed_domains)
    ? settings.agent_web_allowed_domains
    : [];
  ...
  const token = String(
    raw && typeof raw === 'object'
      ? raw.decision ||
          ...
          raw.selection ||
          raw.action ||
          (raw.approved ? 'allow_session' : 'deny')
      : raw,
  )
    .trim()
    .toLowerCase();
  if (['allow_permanent', 'permanent', 'always', 'allow_always'].includes(token))
    return 'allow_permanent';
  if (['allow_all_session', 'allow_all', 'all', 'all_sites', 'unlimited'].includes(token))
...
const VENV_ACTIVATE_PATTERN =
  /\b(?:source|\.)\s+\S*(?:venv|env)\S*[\\/]+(?:bin|scripts)[\\/]+activate\b/i;
  ...
  const token = String(
    raw && typeof raw === 'object'
      ? raw.decision ||
          ...
          raw.selection ||
          raw.action ||
          (raw.approved ? 'approve' : 'deny')
      : raw,
  )
    .trim()
    .toLowerCase();
  if (['approve_permanent', 'permanent', 'always', 'allow_always'].includes(token))
    return 'approve_permanent';
  if (['approve_all', 'allow_all', 'all', 'all_packages', 'unlimited'].includes(token))
    return 'approve_all';
  if (['use_venv', 'venv'].includes(token)) return 'use_venv';
  if (['install_global', 'global', 'globally'].includes(token)) return 'install_global';
  if (
    ['approve', 'allow', 'allow_once', 'once', 'yes', 'ok', 'continue', 'install'].includes(token)
  )
    return 'approve';
  if (['deny', 'denied', 'no', 'reject', 'stop', 'block'].includes(token)) return 'deny';
...
export function createToolApprovalGuards({
  ...
  const requestSessionApproval = async ({
    ...
    const permissionKeys = Array.from(
      new Set(
        [
          grantPermissionKey,
          grantElevated ? 'sudo' : null,
          grantNetwork ? 'network_commands' : null,
        ].filter(Boolean),
      ),
    );
  ...
  const requestSiteApproval = async (host, { action, reason } = {}) => {
    ...
    if (decision === 'allow_permanent') {
      ...
      try {
        ...
        const list = Array.isArray(current.agent_web_allowed_domains)
          ? current.agent_web_allowed_domains.slice()
          : [];
  ...
  const requestPackageApproval = async (pkgToken, { manager }) => {
    ...
    if (
      approvalState.packageApprovedSession instanceof Set &&
      approvalState.packageApprovedSession.has(norm)
    )
      return true;
    if (
      approvalState.packageDeniedSession instanceof Set &&
      approvalState.packageDeniedSession.has(norm)
    )
      return false;
    ...
    if (decision === 'approve_permanent') {
      ...
      try {
        const current = readOrbSettings();
        const list = Array.isArray(current.agent_package_allowed)
          ? current.agent_package_allowed.slice()
          : [];
      ...
      options: [
        {
          ...
          description:
            'Block this global install; the agent should create/use a project .venv instead.',
          recommended: true,
        },
        {
          id: 'install_global',
...
export function createModuleBroker({
  ...
  return {
    ...
    async execute(toolName, args = {}) {
      ...
      const step = Number.isFinite(Number(approvalState?.currentStep))
      ...
      if (!access.available) {
        ...
        const explicitApprovalBlocked =
          access.code === 'explicit_approval_required' ||
          accessReasonLower.includes('explicit approval is required');
        const permissionBlocked =
          access.code === 'permission_required' ||
          accessReasonLower.includes('permission is disabled');
        if (explicitApprovalBlocked && onApprovalRequest && !approvalState.granted) {
          ...
          if (approved) {
            ...
            if (recheck.available) {
              ...
              throw new Error(
                recheck.reason || `Tool is not available in this runtime: ${toolName}`,
              );
            }
          } else {
          ...
          const requestedAction =
            String(safeArgs.action || safeArgs.command || `use ${toolName}`).trim() ||
            `use ${toolName}`;
          ...
          if (approved) {
            ...
            if (
              recheck.code === 'explicit_approval_required' &&
              onApprovalRequest &&
      ...
      if (toolName === 'skills.list') {
        const profile = String(
          safeArgs.profile || resolveActiveSkillProfile(settings) || '',
        ).trim();
      ...
      if (toolName === 'approval.request') {
        ...
        const reason = String(
          safeArgs.reason || safeArgs.description || 'Risky action requires confirmation.',
        ).trim();
      ...
      if (toolName === 'user.ask') {
        ...
        return {
          ...
          answer:
            answer ||
            '(the user did not answer — proceed with your best judgment and note the assumption)',
        };
      }
      ...
      if (toolName === 'files.find') {
        ...
        const fuzzyThreshold = Number.isFinite(Number(safeArgs.fuzzyThreshold))
          ? Number(safeArgs.fuzzyThreshold)
          : 0.42;
        ...
        if (Array.isArray(finalResult?.results) && finalResult.results.length === 0) {
          ...
          for (const fallbackPath of widenedFallbackPaths) {
            ...
            const currentCount = Array.isArray(finalResult?.results)
            ...
            if (
              ...
              (currentCount === 0 && retryScanned > currentScanned)
            ) {
              finalResult = {
                ...retry,
                query,
      ...
      if (toolName === 'rag.retrieve') {
        ...
        const maxPassages = Math.max(
          1,
          Math.min(30, Math.round(Number(safeArgs.maxPassages) || 12)),
        );
      ...
      if (toolName === 'files.read') {
        ...
        const maxChars = Number.isFinite(Number(safeArgs.maxChars))
          ? Number(safeArgs.maxChars)
          : 12000;
        ...
        const tailCount = Number.isFinite(Number(safeArgs.tail))
          ? Math.max(1, Math.round(Number(safeArgs.tail)))
          : 0;

        const readOptions = {};
        if (readPattern) {
          readOptions.pattern = readPattern;
          if (safeArgs.useRegex === true || safeArgs.patternRegex === true)
            readOptions.patternRegex = true;
          ...
          if (Number.isFinite(Number(safeArgs.maxResults)))
            readOptions.maxResults = Number(safeArgs.maxResults);
        } else if (tailCount > 0) {
          ...
          const rendered = blocks
            .map((block) =>
              (Array.isArray(block.lines) ? block.lines : [])
                .map(
                  (l) =>
                    `${showLineNumbers ? `${String(l.line).padStart(width)}\t` : ''}${l.content}`,
                )
                .join('\n'),
            )
            .join('\n     ⋮\n');
        ...
        const chunkLineCount = Number.isFinite(Number(result.lineCount))
          ? Number(result.lineCount)
          : lineCount;
        const chunkStartLine = Number.isFinite(Number(result.startLine))
          ? Number(result.startLine)
          : startLine;
      ...
      if (toolName === 'artifact.create') {
        const rawName = String(safeArgs.filename || safeArgs.name || '')
          ...
          .replace(/^.*[\\/]/, '');
        const filename =
          rawName.replace(/[^A-Za-z0-9._-]/g, '_').slice(0, 120) || `artifact-${Date.now()}.md`;
        const content = String(safeArgs.content || '');
        ...
        const totalBytes = Number.isFinite(Number(saved?.bytes))
          ? Number(saved.bytes)
          : content.length;
        ...
        traceTool.thinking(
          `${append ? 'Appended to' : 'Saved'} artifact ${filename} (${totalBytes} bytes).`,
        );
        return {
          ok: true,
          filename,
      ...
      if (toolName === 'notes.update') {
        ...
        if (safeArgs.content !== undefined)
          updates.content = String(safeArgs.content).slice(0, MAX_NOTE_CONTENT_LENGTH);
      ...
      if (toolName === 'search.web') {
        ...
        if (cachedResult) {
          ...
          traceTool.thinking(
            `Reusing cached web research for "${query}" to avoid duplicate network requests.`,
            {
              step: Number.isFinite(Number(approvalState?.currentStep))
                ? Number(approvalState.currentStep)
                : undefined,
              chart: {
          ...
          return {
            ...
            callsRemaining: Math.max(
              0,
              Number(webSearchState?.maxCalls || 0) - Number(webSearchState?.callsUsed || 0),
            ),
          };
        }
        ...
        const callBudget = Number.isFinite(Number(webSearchState?.maxCalls))
          ...
          : SEARCH_WEB_DEFAULT_CALL_BUDGET;
        const callsUsed = Number.isFinite(Number(webSearchState?.callsUsed))
        ...
        try {
          ...
          const paidFallbackBlocked = searchErrorMessage.includes(
            'paid fallback available but blocked by approval',
          );
          ...
          if (
            ...
            const approvalResponse = await onApprovalRequest({
              ...
              reason:
                searchError?.message || 'Free or non-paid providers were exhausted for this query.',
              stepAction: {
                tool: 'search.web',
                query,
        ...
        const stepHint = Number.isFinite(Number(approvalState?.currentStep))
          ? Number(approvalState.currentStep)
          : undefined;
        const maxLinesRead = Math.max(
          1,
          ...sources.map((source) => Number(source?.linesRead || 0)),
        );
        ...
        const payload = {
          ...
          relatedQueries: Array.isArray(webData?.relatedQueries)
            ? webData.relatedQueries.slice(0, 8)
            : [],
          ...
          visualizer: {
            ...
            linesReadTotal: Number(webData?.linesReadTotal || 0),
            charsReadTotal: Number(webData?.charsReadTotal || 0),
            series: (Array.isArray(webData?.steps) ? webData.steps : [])
              .slice(0, maxSources)
              .map((step) => ({
      ...
      if (toolName === 'launch.run') {
        ...
        if (launcherName) {
          ...
          if (!resolvedCategory)
            resolvedCategory = String(resolvedEntry.category || '').toLowerCase();
        }
        ...
        try {
          ...
          if (canRequest) {
            const approved = await requestSessionApproval({
              ...
              grantElevated: lowerReason.includes('sudo'),
              grantNetwork: lowerReason.includes('network-related commands are blocked'),
              grantShellPassthrough: lowerReason.includes(
                'shell passthrough launch commands are blocked',
              ),
            });
      ...
      if (toolName === 'agent.delegate') {
        ...
        if (delegateResult?.taskId && waitMs !== 0) {
          ...
          try {
            ...
            const _takeover =
              _status === 'timeout' || _status === 'failed' || _status === 'partial'
            ...
            if (!_ok && settings?.agent_planning_mode === true) {
              const reassign = reassignFailedPart(
                String(delegateResult.toAgent || safeArgs.toAgent || ''),
                settings,
              );
              if (reassign && reassign.memberId !== delegateResult.toAgent) {
                ...
                agentState.current = AGENT_STATES.DELEGATED_PAUSE;
                try {
                  const reDelegate = await handleAgentDelegate(
                    { ...safeArgs, toAgent: reassign.memberId },
                    settings,
                  );
      ...
      if (toolName === 'agent.readOutput') {
        ...
        return {
          taskId,
          output:
            output || '(no output file for this task — it may have produced no result or expired)',
          chars: output.length,
        };
      }
          ...
          try {
            ...
            const answer = String(
              response && typeof response === 'object'
                ? (response.answer ?? response.decision ?? '')
                : response || '',
            ).toLowerCase();
      ...
      if (toolName === 'search.ripgrep') {
        ...
        const mode = ['content', 'files', 'count'].includes(
        ...
        const result = await powerRipgrep(pattern, {
          ...
          contextLines: Number.isFinite(Number(safeArgs.contextLines))
            ? Number(safeArgs.contextLines)
            : 2,
          maxResults: Number.isFinite(Number(safeArgs.maxResults))
            ? Number(safeArgs.maxResults)
            : 40,
        });
      ...
      if (toolName === 'memory.query') {
        ...
        const limit = Number.isFinite(Number(safeArgs.limit))
          ? Math.max(1, Math.min(20, Number(safeArgs.limit)))
          : 5;
      ...
      if (toolName === 'chat.recall') {
        ...
        const scope =
          String(safeArgs.scope || 'compacted').toLowerCase() === 'full' ? 'full' : 'compacted';
        let context = '';
        try {
      ...
      if (toolName === 'context.summarize') {
        ...
        const maxChars = Number.isFinite(Number(safeArgs.maxChars))
          ? Math.max(200, Number(safeArgs.maxChars))
          : 2000;
        ...
        try {
          ...
          const tokensSaved = Math.max(
            0,
            Math.ceil(historyDigest.length / 4) - Math.ceil(summary.length / 4),
          );
      ...
      if (toolName === 'search.find') {
        ...
        const result = await powerFind({
          ...
          minSizeKb: Number.isFinite(Number(safeArgs.minSizeKb))
            ? Number(safeArgs.minSizeKb)
            : undefined,
          maxSizeKb: Number.isFinite(Number(safeArgs.maxSizeKb))
            ? Number(safeArgs.maxSizeKb)
            : undefined,
      ...
      if (toolName === 'search.fd') {
        ...
        const result = await powerFd({
          ...
          type: safeArgs.type ? String(safeArgs.type) : undefined,
          hidden: safeArgs.hidden === true,
          maxDepth: Number.isFinite(Number(safeArgs.maxDepth))
            ? Number(safeArgs.maxDepth)
            : undefined,
          exclude: Array.isArray(safeArgs.exclude) ? safeArgs.exclude.map(String) : [],
          maxResults: Number.isFinite(Number(safeArgs.maxResults))
            ? Number(safeArgs.maxResults)
      ...
      if (toolName === 'files.diff') {
        ...
        const contextLines = Number.isFinite(Number(safeArgs.contextLines))
          ? Number(safeArgs.contextLines)
          : 3;
      ...
      if (toolName === 'web.fetch') {
        ...
        if (!url || !/^https?:\/\//.test(url))
          throw new Error('valid https url required for web.fetch');
      ...
      if (toolName === 'terminal.script') {
        ...
        if (!allowedScripts.has(scriptName)) {
          throw new Error(
            `Unknown script "${scriptName}". Available: ${[...allowedScripts].join(', ')}`,
          );
        }
      ...
      if (toolName === 'terminal.pipe') {
        const commands = Array.isArray(safeArgs.commands)
          ? safeArgs.commands.map(String).filter(Boolean)
          : [];
        ...
        const cwd = safeArgs.cwd
          ? assertSafePath(String(safeArgs.cwd), { operation: 'cwd', settings })
          : undefined;
        const timeoutMs = Number.isFinite(Number(safeArgs.timeout))
          ? Math.min(Number(safeArgs.timeout), 120000)
          : 30000;
