 ...
 */
export function normalizeAgentRole(value: unknown, fallback: AgentRoleId = 'executor'): AgentRoleId {
...
export function hasAgentRoleModel(settings: SettingsLike | null | undefined, role: AgentRoleId): boolean {
 ...
 */
export function readAgentRoleAssignments(settings: SettingsLike | null | undefined): AgentRoleAssignments {
 ...
 */
export function resolveAgentIdentity(roleInput: unknown, settings: SettingsLike | null | undefined): AgentIdentity {
  const role = normalizeAgentRole(roleInput)
  ...
  return {
    role,
    provider: explicitlyAssigned ? identityText(binding?.provider) : identityText(settings?.ai_provider),
    model: explicitlyAssigned ? identityText(binding?.model || settings?.ai_model) : identityText(settings?.ai_model),
...
export function applyAgentIdentityToSettings<T extends SettingsLike>(settings: T, identity: AgentIdentity): T {
...
export function resolveCurrentAgentRole(settings: SettingsLike | null | undefined): AgentRoleId {
  ...
  if (provider === 'local' || ['llama', 'gemma', 'phi', 'mistral'].some((family) => model.includes(family))) {
    return 'scout'
  }
  return 'executor'
}
