...
export function normalizeDecision(rawDecision: unknown, rawText: unknown): ControllerDecision {
  ...
  const todoUpdates = Array.isArray(rawDecision.todo_updates) ? rawDecision.todo_updates.filter(isRecord) : []
...
export function mapNativeMetaToDecision(meta: ProviderMeta | null | undefined): ControllerDecision | null {
...
export function looksLikeControllerSchemaText(text: unknown): boolean {
  ...
  return [/"thinking"\s*:/i, /"todo_updates"\s*:/i, /"action"\s*:/i, /"type"\s*:\s*"tool"/i, /"tool"\s*:/i].some(
