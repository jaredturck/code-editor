...
export async function probeModelHealthOnce(settings: Settings): Promise<ModelHealthProbeResult> {
  ...
  try {
    ...
    for (const group of groups.values()) {
      ...
      const available = await discoverModelsForProvider(
        group.provider,
        settings as never,
        group.keyId,
      );
...
function scheduleNext(): void {
  ...
  const delay = hasSuspended
    ? Math.min(MAX_BACKOFF_MS, degradedBase * 2 ** emptyProbeStreak)
    : healthyInterval;
...
export async function retryModelHealthNow(
