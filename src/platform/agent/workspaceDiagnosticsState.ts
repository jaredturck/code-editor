...
function ensure_workspace_listener() {
  ...
  remove_workspace_listener = window.editor_api.workspace.on_change((payload) => {
    ...
    if (!state || path_has_excluded_directory(root, payload.file_path)) return
    state.change_version += 1
    state.dirty = true
  })
}

export function markWorkspaceDiagnosticsDirty(root_path: string) {
  const root = normalize_path(root_path)
  if (!root) return
...
async function scan_workspace(root_path: string): Promise<WorkspaceDiagnosticsSnapshot> {
  ...
  for (let index = 0; index < collected.files.length; index += 12) {
    ...
    for (const entry of results) {
      ...
      analyzed_files += 1
      if (entry.result.diagnostics.length > 0) diagnostic_files += 1
...
async function refresh_state(state: WorkspaceDiagnosticsState, retry = true): Promise<WorkspaceDiagnosticsSnapshot> {
  ...
  const snapshot = await scan_workspace(state.root)
