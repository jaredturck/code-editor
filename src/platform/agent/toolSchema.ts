...
export function buildJsonSchemaTools(
  toolDefinitions:
    | readonly Pick<ToolDefinition, 'name' | 'description' | 'args'>[]
    | null
    | undefined,
): JsonSchemaTool[] {
  return (Array.isArray(toolDefinitions) ? toolDefinitions : [])
    .filter((tool): tool is Pick<ToolDefinition, 'name' | 'description' | 'args'> =>
      Boolean(tool?.name),
    )
...
export function toAnthropicTools(
  jsonTools: readonly JsonSchemaTool[] = [],
