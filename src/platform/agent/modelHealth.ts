...
export function parseModelHealthKey(
  key: string,
...
export function modelHealthSnapshot(): Array<
  { id: string; status: ModelHealthStatus } & ModelHealthEntry
> {
  ensureHydrated();
...
export function resolveFailoverPolicy(
  settings: Record<string, unknown> | null | undefined,
): FailoverPolicy {
  const legacy =
    settings?.agent_failover === false ? 'off' : settings?.agent_failover === true ? 'limited' : '';
  ...
  const attempts =
    Number.isFinite(raw) && raw > 0 ? Math.min(FAILOVER_HARD_CAP, Math.round(raw)) : 4;
