...
export const logInfo = (scope: string, message: string, data?: unknown) => log('info', scope, message, data)
// Records warn through the sanitized application logger.
export const logWarn = (scope: string, message: string, data?: unknown) => log('warn', scope, message, data)
// Records error through the sanitized application logger.
export const logError = (scope: string, message: string, data?: unknown) => log('error', scope, message, data)
// Records event through the sanitized application logger.
export const logEvent = (scope: string, message: string, data?: unknown) => log('event', scope, message, data)
...
export async function showLogs(): Promise<{
  ...
  if (typeof window === 'undefined') return null
  const fn = (window.orbitDesktop as unknown as { showLogs?: () => Promise<unknown> } | undefined)?.showLogs
  if (typeof fn !== 'function') return { ok: false, error: 'Logs are only available in the desktop app.' }
  try {
...
function captureGlobalErrors(): void {
  ...
  window.addEventListener('unhandledrejection', (event) => {
    ...
    logError('renderer', `Unhandled rejection: ${reason instanceof Error ? reason.message : String(reason)}`, {
