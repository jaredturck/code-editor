...
function applyGenericRules(evaluation: ModelEvaluation): void {
  ...
  if (
    /deprecated|legacy|gpt-3\.5|gemini-1\.5|gemini-2\.0|claude-3[-.]|codellama|phi3\b/.test(model)
  ) {
    addAllRoles(evaluation, -55);
    evaluation.quality -= 35;
    evaluation.reasons.push(
      'Legacy generation; kept available manually but avoided by auto setup.',
    );
  }
...
export function compareForRole(
  role: AgentRoleId,
  left: ModelEvaluation,
  right: ModelEvaluation,
): number {
  return (
