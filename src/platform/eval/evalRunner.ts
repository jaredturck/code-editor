...
export function aggregateReport(results: EvalResult[]): EvalReport {
  ...
  return {
    ...
    avgPromptTokens: Math.round(
      _avg(usages.map((usage) => _num(usage.promptTokens ?? usage.lastPromptTokens))),
    ),
    avgCompletionTokens: Math.round(_avg(usages.map((usage) => _num(usage.completionTokens)))),
    avgCacheHitRatio:
      Math.round(_avg(usages.map((usage) => _num(usage.cacheHitRatio))) * 100) / 100,
    avgNativeAdoption:
      Math.round(_avg(usages.map((usage) => _num(usage.nativeToolAdoption))) * 100) / 100,
    ...
    totalInvalidArgErrors: summaries.reduce(
      (total, summary) => total + _num(summary.invalidArgErrors),
      0,
    ),
    totalRedundantCalls: summaries.reduce(
      (total, summary) => total + _num(summary.redundantToolCalls),
      0,
    ),
...
export async function runEvalSuite({
  ...
  if (typeof runSession !== 'function')
    throw new Error('runEvalSuite requires a runSession function');
