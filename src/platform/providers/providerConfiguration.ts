...
export function getProviderCatalog(
  ...
  const discovered = validKeyIds.flatMap((keyId) => getDiscoveredModelsForKey(settings, providerId, keyId))
  if (discovered.length) return normalizeModelList(discovered).sort((a, b) => a.localeCompare(b))
  return normalizeModelList(settings?.discovered_models?.[providerId]).sort((a, b) => a.localeCompare(b))
}
...
export function getReadyProviderIds(settings: ProviderConfigurationSettings | null | undefined): string[] {
