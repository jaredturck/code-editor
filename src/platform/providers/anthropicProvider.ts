...
export function normalizeAnthropicMessages(messages: readonly AIMessage[]): Array<Record<string, unknown>> {
...
export function resolveAnthropicThinkingConfig(model: string, settings: AISettings): AnthropicThinkingConfig {
  const extendedThinking = settings?.extended_thinking === true
  const thinkingBudget = Math.max(1000, Math.min(16000, Number(settings?.thinking_budget_tokens || 8000)))
  // New adaptive-thinking models reject the legacy fixed-budget shape; older Claude models retain it.
  const adaptiveThinking = /claude-(?:opus-4-[6-9]|sonnet-4-[6-9])/.test(model) || /fable/.test(model)
...
export function buildAnthropicRequestBody(
  ...
  const systemText = systemMessage ? contentToText(systemMessage.content) : 'You are a helpful AI assistant.'
  const requestBody: Record<string, unknown> = {
    model,
    // Legacy thinking reserves answer room after its budget; adaptive thinking does not.
    max_tokens: Math.min(
      outputCeiling,
      thinking.extendedThinking && thinking.legacyThinking ? thinking.thinkingBudget + maxOutput : maxOutput,
    ),
    system: [{ type: 'text', text: systemText, cache_control: { type: 'ephemeral' } }],
    messages: normalizeAnthropicMessages(messages),
  }

  const tools = Array.isArray(options.tools) && options.tools.length ? toAnthropicTools(options.tools) : null
  if (tools) {
...
export function buildAnthropicHeaders(apiKey: string, model: string, settings: AISettings): Record<string, string> {
  const thinking = resolveAnthropicThinkingConfig(model, settings)
  return {
...
export function finalizeAnthropicStreamToolCalls(blocks: Map<number, AnthropicStreamBlock>): ToolCall[] {
...
export function parseAnthropicResponse(data: AnthropicResponse, model: string): ProviderMeta {
  ...
  const toolCalls = content
    ...
    .map((block) => ({
      ...
      argsError: data?.stop_reason === 'max_tokens' && (!block.input || Object.keys(block.input).length === 0),
    }))
 ...
 */
export async function listAnthropicModels(apiKey: unknown, fetchFn: ProviderFetch): Promise<string[]> {
  const _apiKey = normalizeApiKey(apiKey)
  if (!_apiKey) return []
