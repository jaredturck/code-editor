...
export function estimateLocalModelMemoryGb(model: unknown): number | null {
  ...
  const bytesPerParameter = QUANTIZATION_BYTES_PER_PARAMETER[quantization] || DEFAULT_QUANTIZED_BYTES_PER_PARAMETER
  return Math.round(parameters * bytesPerParameter * RUNTIME_OVERHEAD_RATIO * 10) / 10
}

export function availableLocalVramGb(hardware: LocalHardwareProfile | null | undefined): number | null {
