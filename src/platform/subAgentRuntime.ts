...
import type { STPTask, STPStep } from '@/platform/stpBuilder';
 ...
 * @returns {Promise<object[]>}
 */
export function waitForAllTasks(
...
const SUB_BLOCKED_WRITE = [
  /^\/(etc|boot|sys|proc|usr|bin|sbin|lib|dev)(\/|$)/i,
  /(^|\/)\.ssh(\/|$)/i,
  /\/etc\//i,
];
...
const SUB_PIPE_TO_SHELL =
  /(curl|wget|fetch)\s+[^|;]*\|\s*(sudo\s+)?(sh|bash|zsh|python|perl|node)\b/i;
...
function assertSubAgentPathSafe(
  ...
  if (
    (operation === 'write' || operation === 'cwd') &&
    SUB_BLOCKED_WRITE.some((p) => p.test(normalized))
  ) {
...
function assertSubAgentCommandSafe(
  command: unknown,
  settings: SubAgentSettings,
  tier: number,
): string {
  ...
  if (SUB_DANGEROUS_COMMAND.some((p) => p.test(text)))
    throw new Error('Command blocked by safety policy.');
  if (SUB_PIPE_TO_SHELL.test(text))
  ...
  if (safety.blockSudo && SUB_SUDO.test(text))
...
async function executeSubAgentTool(
  ...
  if (SUB_AGENT_PEER_TOOLS.includes(name)) {
    ...
    if (!isMeshEnabled(settings)) {
      return {
        error:
          'Peer tools are off. Enable the communication bridge + peer consultation in Settings → Agents.',
      };
    }
  ...
  if (
    (name === 'files.write' || name === 'files.patch' || name === 'files.edit') &&
    !cap.fileWrite
  ) {
  ...
  switch (name) {
    ...
    case 'launch.run': {
      ...
      const cwd = safeArgs.cwd
        ? assertSubAgentPathSafe(String(safeArgs.cwd), 'cwd', settings)
        : undefined;
      ...
      if (launchName) {
        ...
        if (!entry) {
          throw new Error(
            `No launcher named "${launchName}". Use a raw command or check the main launcher menu.`,
          );
        }
      ...
      return launchLocalCommand(
        assertSubAgentCommandSafe(commandInput, settings, tier),
        category,
        cwd,
      );
    }
    ...
    case 'web.fetch': {
      ...
      return powerWebFetch(url, {
        ...
        ...(Number.isFinite(Number(safeArgs.maxChars))
          ? { maxChars: Number(safeArgs.maxChars) }
          : {}),
      });
    }
    ...
    case 'skills.search': {
      ...
      const matches = skills
        ...
        .filter((s) => {
          ...
          const hay =
            `${s.id} ${s.title} ${s.summary} ${(s.triggers || []).join(' ')}`.toLowerCase();
        ...
        .map((s) => {
          ...
          const text =
            stp.skills.variant === 'simple'
              ? modelVariants.simple || s.instructions || ''
              : s.instructions || '';
          return {
            id: s.id,
            title: s.title,
...
function buildSubAgentNativeTools(stp: STPTask, settings: SubAgentSettings = {}): JsonSchemaTool[] {
  ...
  const available = explicit
    ? requested
    : listSubAgentNativeToolNames().filter(
        (name) => getSubAgentMinTier(name, PERMISSION_TIER.STANDARD) <= tier,
      );
 ...
 */
export function buildSubAgentModelMessages(
  stp: STPTask,
  systemPrompt: string,
...
export async function executeSTP(
  ...
  let softDeadline =
    startedAt + Math.max(8000, Math.round(taskTimeoutMs * SUB_AGENT_DEADLINE_RATIO));
  ...
  for (let step = 0; ; step += 1) {
    ...
    try {
      ...
      const thought =
        isRecord(parsed) && typeof parsed.thinking === 'string' ? parsed.thinking.trim() : '';
      if (thought)
        emit({
          type: 'thinking',
    ...
    while (running) {
      ...
      const taskEmit: SubAgentEventEmitter = (event) =>
        emitSubAgentEvent({
          ...
          agentId,
          model: String(
            (event as { model?: unknown }).model || nextTask.agentIdentity?.model || '',
          ),
