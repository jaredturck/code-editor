...
const SKILL_ALIGNMENT: Record<string, SkillAlignmentRule> = {
  ...
  'ui-conventions': {
    tools: [],
    keywords: ['aria-', 'accessibility', 'wcag', 'tailwind', 'css', 'component', 'responsive', 'button', 'form'],
  },
  'ai-code-utilization': {
...
export function scoreSession({
  ...
  const details: SkillRewardDetail[] = triggeredSkillIds.map((id) => {
    ...
    const toolHit = alignment.tools.length === 0 || alignment.tools.some((tool) => toolSet.has(tool))
    const kwHit = alignment.keywords.length === 0 || alignment.keywords.some((keyword) => replyLower.includes(keyword))
...
export function recordDelegationMetrics(metrics: Partial<DelegationMetrics>): void {
  ...
  delegationMetrics.escalationRate = total > 0 ? Math.round((delegationMetrics.escalations / total) * 100) / 100 : 0
...
export function buildSessionDebrief(rewardStore: RewardDebriefStore, modelFamily: string): string | null {
  const recent = (rewardStore.sessions || []).slice(0, 3)
  if (!recent.length) return null

  const avgScore = recent.reduce((sum, session) => sum + Number(session.score || 0), 0) / recent.length
  ...
  const topMissed = Object.entries(familyCounts)
    ...
    .sort(
      ([, a], [, b]) =>
        Number(b.missed || 0) / Number(b.triggered || 1) - Number(a.missed || 0) / Number(a.triggered || 1),
    )
    .slice(0, 3)
    .map(([id]) => id)
