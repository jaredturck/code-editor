...
export function isLoopbackAddress(address: string, family?: number): boolean {
  ...
  return detected === 4
    ? loopbackIpv4BlockList.check(address, 'ipv4')
    : loopbackIpv6BlockList.check(address, 'ipv6');
}
...
export function isBlockedRemoteAddress(address: string, family?: number): boolean {
  ...
  return (
    !publicIpv6AddressList.check(address, 'ipv6') || blockedIpv6AddressList.check(address, 'ipv6')
  );
}
...
export function normalizeRemoteRequestHeaders(
  ...
  for (const [rawName, rawValue] of Object.entries(headers || {})) {
    ...
    if (!name || !allowed.has(lower)) {
      throw withStatus(
        `Request header is not allowed by the remote destination policy: ${name}`,
        400,
      );
    }
...
export async function resolveAndValidateRemoteUrl(
  ...
  if (
    ...
    !policy.allowedHosts.some((pattern) => hostMatches(hostname, pattern))
  ) {
  ...
  for (const entry of addresses) {
    ...
    if (blocked) {
      throw withStatus(
        'Remote destination resolves to a private, link-local, or reserved address',
        403,
      );
    }
  }
...
function createPinnedLookup(addresses: readonly ResolvedAddress[]): LookupFunction {
  return (_hostname, options, callback) => {
    ...
    const requestedFamily =
      familyOption === 'IPv4' ? 4 : familyOption === 'IPv6' ? 6 : Number(familyOption || 0);
...
export async function openSafeRemoteResponse(
  ...
  const resolveBeforeDeadline = async (target: string | URL) => {
    ...
    return new Promise<Awaited<ReturnType<typeof resolveAndValidateRemoteUrl>>>(
      (resolve, reject) => {
        const onAbort = () => {
          reject(abortedError());
        };
  ...
  const requestTarget = async (
    ...
    const response = await new Promise<IncomingMessage>((resolve, reject) => {
      ...
      if (bodyInput !== undefined && method !== 'GET' && method !== 'HEAD')
        request.write(bodyInput);
      request.end();
    });
...
export async function safeRemoteRequestBuffer(
  ...
  const maxResponseBytes = Math.max(
    1,
    Number(policy.maxResponseBytes) || DEFAULT_MAX_RESPONSE_BYTES,
  );
