...
export async function getAudioTranscriptionStatus(): Promise<NoteTranscriptionStatus> {
  ...
  try {
    ...
    const modelInstalled = Array.isArray(payload.models)
      ? payload.models.some((model) => modelNameMatches(model.name) || modelNameMatches(model.model))
      : false
    return {
...
async function transcribeWithLocalGranite(audio: Buffer, signal?: AbortSignal): Promise<AudioTranscriptionResult> {
  const status = await getAudioTranscriptionStatus()
  ...
  try {
    ...
    const text =
      payload && typeof payload === 'object' ? String((payload as Record<string, unknown>).text || '').trim() : ''
