...
function isClipDependencyFailure(error: unknown): boolean {
  return /cannot find (package|module)|module_not_found|onnxruntime|sharp/i.test(
    errorMessage(error),
  );
}
...
async function loadClipVisionModel(
  transformers: any,
  dtype: 'fp16',
  deviceIndex: number,
): Promise<any> {
...
export async function hasCompleteFileClipModelCache(
  cacheDirectory = FILE_CLIP_CACHE_DIR,
): Promise<boolean> {
...
export function createClipRawImages(
  preparedImages: PreparedClipImage[],
  RawImage: ClipRuntime['RawImage'],
): any[] {
  return preparedImages.map(
    (image) => new RawImage(image.data, image.width, image.height, image.channels),
  );
}
...
export function directClipPreprocessParameters(
  processor: any,
): DirectClipPreprocessParameters | null {
  const imageProcessor = processor?.image_processor ?? processor?.feature_extractor ?? processor;
  if (
    !imageProcessor ||
    imageProcessor.do_rescale === false ||
    imageProcessor.do_normalize === false
  ) {
    return null;
  }
  if (
    imageProcessor.do_pad ||
    imageProcessor.do_flip_channel_order ||
    imageProcessor.do_convert_grayscale
  ) {
    return null;
  }
...
export function clipVisionLaneCount(imageCount: number, availableLaneCount: number): number {
  return Math.max(
    1,
    Math.min(availableLaneCount, Math.floor(imageCount / MIN_IMAGES_PER_CLIP_LANE)),
  );
}
...
export async function embedClipImages(
  imagePaths: string[],
  signal?: AbortSignal,
): Promise<number[][]> {
