  ...
  private createWorkerSlot(): WorkerSlot {
    ...
    slot.worker.on('exit', (code) => {
      if (!this.closed && code !== 0 && this.workers.includes(slot)) {
        this.replaceFailedWorker(slot, extractionError(`File extraction worker exited with code ${code}`))
      }
    })
    return slot
  }
  ...
  private dispatch(): void {
    ...
    for (const slot of this.workers) {
      ...
      slot.timeout = setTimeout(() => {
        this.replaceFailedWorker(slot, extractionError('File extraction exceeded the six-second limit'))
