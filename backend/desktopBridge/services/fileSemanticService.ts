...
import {
  ...
  type FileImageProcessingPool,
} from './fileImageProcessingPool.js';
...
function updateExtractedStageProgress(
  processed: number,
  indexed: number,
  finalized: number,
  fileTotal: number,
...
function updateEmbeddingEta(completed: number, total: number): void {
  ...
  const windowDuration = recentEmbeddingBatchTimings.reduce(
...
async function pullModel(model: string): Promise<void> {
  ...
  if (!response.ok)
    throw await responseError(response, `Ollama model download failed for ${model}`);
...
function normalizedSearchCandidates(
  ...
  const candidates = records
    ...
    .sort(
      (left, right) =>
        right.rawScore - left.rawScore || left.record.name.localeCompare(right.record.name),
    );
  ...
  return candidates.map((candidate, index) => {
    const zScore =
      scale > 0.000001 ? Math.max(-8, Math.min(8, (candidate.rawScore - center) / scale)) : 0;
...
function nestedMountPaths(source: FileIndexSource, discovered: FileIndexSource[]): Set<string> {
  return new Set(
    discovered
      .filter(
        (candidate) =>
          candidate.path !== source.path && candidate.path.startsWith(`${source.path}${path.sep}`),
      )
...
export async function countEligibleFilesystemEntries(
  ...
  const sourceCounts = await Promise.all(
    sources.map((source) => countEligibleSourceEntries(source, discovered)),
  );
...
function existingNodesByPath(
  ...
  for (const node of nodes) {
    ...
    const key =
      node.id === rootNodeId ? 'virtual' : sourceNodeKey(metadata.sourceId, metadata.relativePath);
    result.set(key, {
      id: node.id,
  ...
  for (const source of sources) {
    ...
    while (stack.length) {
      ...
      for (const entry of entries) {
        ...
        const fileChanged =
          !reusable ||
          reusable.metadata.size !== size ||
          reusable.metadata.modifiedAt !== stats.mtimeMs;
...
function embeddingProfileIsCurrent(
  value: Record<string, unknown> | null,
): value is FileEmbeddingProfile {
  return Boolean(
    value &&
...
async function embeddingBatchFits(
  inputs: string[],
  batchSize: number,
  signal: AbortSignal,
): Promise<boolean> {
...
async function reduceEmbeddingProfile(
...
async function processTextFiles(
  ...
  try {
    ...
    await Promise.all([
      persistence.drain().catch(() => undefined),
      pageTask.catch(() => undefined),
    ]);
  }
...
async function prepareExtractedFile(
  ...
  return {
    ...
    metadata: {
      ...
      ...('archiveEntry' in extracted && extracted.archiveEntry
      ...
      inputFormatVersion: TEXT_EMBEDDING_INPUT_VERSION,
      sourceInputFormatVersion:
        sourceKind === 'pdf' ? PDF_INPUT_FORMAT_VERSION : DOCUMENT_INPUT_FORMAT_VERSION,
    },
  };
}
  ...
  while (offset < prepared.length) {
    ...
    try {
      ...
      await writeEncryptedFileSemantics(
        createExtractedSemanticRecords(group, embeddings, generatedAt),
      );
      offset += group.length;
    } catch (error) {
      if (signal.aborted) throw new FileIndexCancelledError();
...
async function processExtractedTextFiles(
  ...
  const currentBatchSize = () =>
    Math.max(1, Math.min(profile.batchSize, EXTRACTED_EMBEDDING_BATCH_SIZE));
...
async function processDocumentAndPdfStages(
  ...
  try {
    const documents = await processDocumentFiles(
      snapshot,
      initialProfile,
      generation,
      signal,
...
async function processImageFiles(
  ...
  try {
    for await (const outcome of preparedImageOutcomes(
      snapshot,
      pool,
      maximumPending,
      generation,
...
async function buildInitialIndex(
  ...
  const previousRootNodeId = indexMetaIsCurrent(previousMeta)
    ? String(previousMeta?.rootNodeId || '')
    : '';
  runtimeIndexState = {
    ...emptyRuntimeState(),
    status: 'building',
  ...
  try {
    const snapshot = await collectFilesystemSnapshot(
      sources,
      discovered,
      rootNodeId,
      new Map(),
    ...
    const { documents, pdfs } = await processDocumentAndPdfStages(
      snapshot,
      text.profile,
      generation,
      signal,
    ...
    const semanticCount =
      text.semanticCount +
      documents.semanticCount +
      pdfs.semanticCount +
      images.semanticCount +
      videos.semanticCount;
    const failedCount =
      text.failedCount +
      documents.failedCount +
      pdfs.failedCount +
      images.failedCount +
      videos.failedCount;
    ...
    await deleteEncryptedFileConceptGenerationsExcept(
      String(previousMeta?.conceptGeneration || ''),
    ).catch(() => undefined);
...
async function buildRescan(
  ...
  const deletedNodeIds = existingNodes
    .map((node) => node.id)
    .filter((id) => !snapshot.reusedNodeIds.has(id));
  ...
  try {
    ...
    await deleteEncryptedFileConceptGenerationsExcept(String(meta.conceptGeneration || '')).catch(
      () => undefined,
    );
    throw error;
  }
...
function startIndexOperation(
  ...
  activeIndexPromise = (async () => {
    ...
    .catch(async (error) => {
      const cancelled =
        error instanceof FileIndexCancelledError ||
      ...
      runtimeIndexState = {
        ...
        error: cancelled
          ...
          : error instanceof Error
            ? error.message
            : 'Semantic filesystem indexing failed',
      };
    })
    .finally(() => {
...
function statusFromMeta(
  ...
  return {
    ...
    semanticCount: runtimeBuilding
      ? runtimeIndexState.semanticCount
      : Number(currentMeta?.semanticCount || 0),
    conceptCount: runtimeBuilding
      ? runtimeIndexState.conceptCount
      : Number(currentMeta?.conceptCount || 0),
    skippedCount: runtimeBuilding
      ? runtimeIndexState.skippedCount
      : Number(currentMeta?.skippedCount || 0),
    failedCount: runtimeBuilding
      ? runtimeIndexState.failedCount
      : Number(currentMeta?.failedCount || 0),
...
export async function preflightFileSemanticIndex(
  ...
  const forceRefresh =
    typeof selectedSourceIdsOrForce === 'boolean' ? selectedSourceIdsOrForce : force;
...
async function loadSearchRecords(): Promise<SearchCacheRecord[]> {
  ...
  const recordFor = (
    ...
    return {
      ...
      sourceSemanticId,
      embeddingSpace: String(
        metadata.embeddingSpace ||
          (semanticType === 'text' ? FILE_EMBEDDING_MODEL : FILE_CLIP_MODEL),
      ),
    };
  };
...
function bestVisualRecordPerFile(
  records: SearchCacheRecord[],
  embedding: number[],
): SearchCacheRecord[] {
...
export async function searchFileSemanticIndex(
  ...
  const normalizedKind: FileSemanticSearchKind =
    kind === 'text' || kind === 'image' || kind === 'video' ? kind : 'all';
  const textRecords = records.filter(
    (record) =>
      (normalizedKind === 'all' || normalizedKind === 'text') && record.semanticType === 'text',
  );
  const visualRecords = records.filter(
    (record) =>
      record.semanticType !== 'text' &&
      (normalizedKind === 'all' || record.semanticType === normalizedKind),
  );
  ...
  const bestVisualRecords = visualRecords.length
    ? bestVisualRecordPerFile(visualRecords, visualEmbedding)
    : [];
...
function resultWithoutEmbedding(
  record: SearchCacheRecord,
  score: number,
  rawScore?: number,
): FileSemanticSearchResult {
  const {
...
export async function findSimilarFiles(
  ...
  const sources = records.filter(
    (record) => normalizedFilesystemPath(record.path) === normalizedPath,
  );
...
function diverseConceptCandidates(
...
export async function searchFileSemanticConcepts(
  ...
  const concepts = (await readEncryptedFileConcepts(generation)).filter(
    (concept) => concept.memberCount >= 2,
  );
  ...
  for (const candidate of selected) {
    ...
    for (const membership of groupMemberships) {
      const record =
        recordsBySource.get(membership.sourceSemanticId) || recordsByFile.get(membership.fileId);
