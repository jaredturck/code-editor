...
async function discoverDesktopApplications(
  ...
  for (const directory of directories) {
    ...
    for (const entry of entries) {
      ...
      const application = await readDesktopApplication(path.join(directory, entry.name), entry.name, env)
...
async function steamLibraryDirectories(homeDir: string): Promise<string[]> {
  ...
  for (const root of steamRoots(homeDir)) {
    ...
    const content = await fs.readFile(path.join(root, 'steamapps', 'libraryfolders.vdf'), 'utf8').catch(() => '')
...
function metadataFromStored(record: EncryptedLauncherApplicationRecord): LauncherIndexedApplication {
  ...
  return {
    ...
    args: Array.isArray(metadata.args) ? metadata.args.map((value) => String(value)).slice(0, 80) : [],
...
export async function getLauncherSemanticStatus(buildIfMissing = false): Promise<LauncherSemanticStatus> {
  ...
  if (modelState.installed && buildIfMissing && !currentMeta && runtimeIndexState.status !== 'building') {
    startIndexBuild()
  }
...
export async function installLauncherSemanticModel(
  ...
  if (!refreshed.installed) throw new Error('Ollama did not report the embedding model as installed')
  runtimeIndexState = {
    status: 'missing',
    stage: '',
    completed: 0,
...
export async function rebuildLauncherSemanticIndex(): Promise<LauncherSemanticStatus> {
  const modelState = await readOllamaModelState(true)
  if (!modelState.available) throw new Error('The system Ollama service is not available')
  if (!modelState.installed) throw new Error(`Ollama model ${LAUNCHER_EMBEDDING_MODEL} is not installed`)
