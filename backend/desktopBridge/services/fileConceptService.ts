...
function adaptiveConceptCount(fileCount: number, sampleCount: number): number {
  ...
  return Math.max(2, Math.min(MAX_CONCEPTS_PER_SPACE, desired, fileCount, Math.max(2, Math.floor(sampleCount / 4))))
}
...
function adaptiveBroadCount(conceptCount: number, sampleCount: number): number {
  ...
  return Math.max(
    2,
    Math.min(MAX_BROAD_CENTROIDS, Math.ceil(conceptCount / 12), Math.max(2, Math.floor(sampleCount / 8))),
  )
}

function initialCentroids(sample: Float32Array, dimension: number, centroidCount: number, seed: number): Float32Array {
  ...
  for (let centroid = 0; centroid < centroidCount; centroid += 1) {
    ...
    centroids.set(sample.subarray(row * dimension, row * dimension + dimension), centroid * dimension)
  }
...
function representativeVideoFrames(records: EncryptedFileConceptVectorRecord[]): EncryptedFileConceptVectorRecord[] {
...
function localClusterCounts(assignments: Int32Array, broadCount: number, targetConceptCount: number): Int32Array {
  ...
  for (let broad = 0; broad < broadCount; broad += 1) {
    ...
    result[broad] = Math.max(1, Math.min(MAX_LOCAL_CENTROIDS, proportional, Math.max(1, Math.floor(members / 3))))
  }
  return result
}
...
async function trainModel(
  ...
  let broadCentroids = initialCentroids(sample.vectors, sample.dimension, broadCount, hashSeed(space))
  ...
  for (let iteration = 0; iteration < BROAD_ITERATIONS; iteration += 1) {
    ...
    for (let centroid = 0; centroid < broadCount; centroid += 1) {
      ...
      sums.set(
        sample.vectors.subarray(sourceRow * sample.dimension, sourceRow * sample.dimension + sample.dimension),
...
function flattenPage(records: EncryptedFileConceptVectorRecord[], dimension: number): AssignmentPage {
...
async function assignSpace(
  ...
  await pool.setModel(model.dimension, model.broadCentroids, model.localCentroids, model.localOffsets)
...
export async function rebuildFileConceptIndex(options: {
  ...
  try {
    ...
    return {
      ...
      miniLmConceptCount: finalizedConcepts.filter((concept) => concept.embeddingSpace === 'minilm').length,
      clipConceptCount: finalizedConcepts.filter((concept) => concept.embeddingSpace === 'clip').length,
      sourceVectorCount,
      workerCount: pool.workerCount,
    }
  } finally {
