...
function sourceId(uuid: string, mountPath: string, filesystemRoot: string): string {
  if (uuid) {
    ...
    if (normalizedRoot && normalizedRoot !== '/') {
      const rootFingerprint = createHash('sha256').update(normalizedRoot).digest('hex').slice(0, 12)
...
function mountPathIsExcluded(mountPath: string): boolean {
  return [...EXCLUDED_MOUNT_PATHS].some(
    (excludedPath) => mountPath === excludedPath || mountPath.startsWith(`${excludedPath}${path.sep}`),
  )
}
...
async function linuxMountSources(homePath: string): Promise<FileIndexSource[]> {
  ...
  try {
    ...
    findmnt = flattenEntries((JSON.parse(result.stdout) as { filesystems?: FindmntEntry[] }).filesystems || [])
  } catch {
    return []
  }
  try {
    const result = await runProcess(
      'lsblk',
      ['--json', '--bytes', '--output', 'PATH,NAME,TYPE,FSTYPE,SIZE,MOUNTPOINTS,RM,RO,TRAN,UUID,LABEL'],
      { timeoutMs: 5000, maxBufferBytes: 4 * 1024 * 1024 },
    )
    blocks = flattenEntries((JSON.parse(result.stdout) as { blockdevices?: LsblkEntry[] }).blockdevices || [])
  } catch {
    blocks = []
  }
  ...
  for (const entry of findmnt) {
    ...
    if (canonicalHome === mountPath || canonicalHome.startsWith(`${mountPath}${path.sep}`)) continue
    ...
    const removable =
      !network && (blockType === 'rom' || booleanValue(block?.rm) || REMOVABLE_TRANSPORTS.has(transport))
    const readOnly =
...
export function fileIndexSourcesFromMeta(meta: Record<string, unknown> | null): FileIndexSource[] {
  ...
  return values.map(normalizeStoredSource).filter((source): source is FileIndexSource => Boolean(source))
}
...
export async function resolveSelectedFileIndexSources(
  ...
  const sources = discovered.filter(
    (source) => source.alwaysSelected || requested.has(source.id) || (useDefaults && source.selectedByDefault),
  )
...
export async function getFileIndexAccessRoots(homePath: string): Promise<string[]> {
  const [meta, discovered] = await Promise.all([readEncryptedFileIndexMeta(), discoverFileIndexSources(homePath)])
...
export async function getFileIndexSourceState(homePath: string): Promise<{
  ...
  const [discovered, meta] = await Promise.all([discoverFileIndexSources(homePath), readEncryptedFileIndexMeta()])
  const stored = fileIndexSourcesFromMeta(meta)
  const locked = Boolean(meta && meta.status === 'complete' && stored.length)
  if (!locked) {
    return {
      sources: discovered,
      selectedSourceIds: discovered.filter((source) => source.selectedByDefault).map((source) => source.id),
      locked: false,
    }
  }
