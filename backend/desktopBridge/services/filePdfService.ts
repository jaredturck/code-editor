...
import { getDocument, PDFDataRangeTransport, type PDFDocumentLoadingTask } from 'pdfjs-dist/legacy/build/pdf.mjs'
...
async function openPdfRangeTransport(filePath: string): Promise<FileRangeTransport> {
  ...
  try {
    ...
    return new FileRangeTransport(fileHandle, fileStat.size, Uint8Array.from(initialBuffer.subarray(0, bytesRead)))
  } catch (error) {
 ...
 */
export async function extractPdfText(filePath: string, signal: AbortSignal): Promise<PdfTextResult | null> {
