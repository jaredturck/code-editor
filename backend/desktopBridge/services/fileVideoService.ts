...
export function videoProbeTimestamps(durationSeconds: number): number[] {
  ...
  const interval =
    durationSeconds <= 60 ? 0.5 : durationSeconds <= 600 ? 1 : durationSeconds / MAX_PROBES;
...
async function extractRawFrames(
  ...
  for (let start = 0; start < timestamps.length; start += FRAME_CHUNK_SIZE) {
    ...
    filters.push(
      `${chunk.map((_, index) => `[v${index}]`).join('')}concat=n=${chunk.length}:v=1:a=0[out]`,
    );
    args.push(
      '-filter_complex',
      filters.join(';'),
    ...
    if (result.exitCode !== 0)
      throw new Error(result.stderr.trim() || 'FFmpeg frame extraction failed');
...
function selectDistinctiveTimestamps(
  ...
  const candidates = signatures
    .map((current, index) => {
      if (index === 0 || current.brightness < 5 || current.brightness > 250 || current.variance < 3)
        return null;
      ...
      return {
        ...
        score: Math.max(
          signatureDifference(current, previous),
          signatureDifference(current, older),
        ),
      };
    })
  ...
  for (const candidate of candidates) {
    if (selected.some((value) => Math.abs(value - candidate.timestampMs) < MIN_SCENE_GAP_MS))
      continue;
...
export async function extractVideoFramesForIndex(
  ...
  const probeFrames = await extractRawFrames(
    filePath,
    probeTimes,
    PROBE_WIDTH,
    PROBE_HEIGHT,
  ...
  const signatures = probeFrames.map((frame, index) =>
    signature(frame, Math.round((probeTimes[index] || 0) * 1000)),
  );
