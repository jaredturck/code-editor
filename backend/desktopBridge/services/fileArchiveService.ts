...
function isEligibleZipEntry(name: string, originalSize?: number): boolean {
  ...
  return Boolean(
    normalized &&
    !normalized.endsWith('/') &&
    zipPathDepth(normalized) <= ZIP_SAMPLE_MAX_DEPTH &&
    originalSize !== 0,
  );
}
...
function bestZipCandidate(
  candidates: ZipTextCandidate[],
  inspectedEntries: number,
): GenericZipTextResult | null {
  candidates.sort(
    (left, right) => right.score - left.score || left.entryName.localeCompare(right.entryName),
  );
