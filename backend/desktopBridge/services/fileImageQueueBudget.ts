...
export function calculatePreparedImageQueueBudget(
  totalMemoryBytes: number,
  availableMemoryBytes: number,
): number {
  const reserveBytes = clamp(
    Math.floor(totalMemoryBytes * 0.25),
...
export function calculatePreparedImageQueueCapacity(
  ...
  const normalizedBatchCount = Math.max(
    2,
    Math.min(MAXIMUM_PREPARED_BATCH_COUNT, Math.floor(targetBatchCount)),
  );
  const queueBudgetBytes = calculatePreparedImageQueueBudget(
    totalMemoryBytes,
    availableMemoryBytes,
  );
  const memoryCapacity = Math.max(
    normalizedWorkers,
    Math.floor(queueBudgetBytes / ESTIMATED_PREPARED_IMAGE_BYTES),
  );
...
export async function resolvePreparedImageQueueCapacity(
  workerCount: number,
  batchSize = 256,
): Promise<number> {
  const availableMemoryBytes = (await linuxAvailableMemory()) ?? os.freemem();
  return calculatePreparedImageQueueCapacity(
    os.totalmem(),
    availableMemoryBytes,
    workerCount,
    batchSize,
