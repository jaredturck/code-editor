...
function topBroadIndexes(vectors: Float32Array, vectorOffset: number, maximum: number): number[] {
  ...
  for (let broad = 0; broad < broadCount; broad += 1) {
    const score = conceptDotRows(vectors, vectorOffset, modelBroadCentroids, broad * modelDimension, modelDimension)
    if (score > firstScore) {
...
port.on('message', (request: FileConceptWorkerRequest) => {
  try {
    ...
    port.postMessage(response, [conceptIndexes.buffer as ArrayBuffer, scores.buffer as ArrayBuffer])
  } catch (error) {
    const response: FileConceptWorkerResponse = {
      id: request.id,
      type: 'error',
