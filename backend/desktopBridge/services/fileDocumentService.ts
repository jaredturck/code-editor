...
function sortedEntries(entries: Iterable<Entry>, pattern: RegExp): Entry[] {
  return [...entries]
    ...
    .sort(
      (left, right) =>
        entryNumber(left.fileName) - entryNumber(right.fileName) ||
...
async function extractPptx(pkg: ZipPackage, signal: AbortSignal): Promise<string> {
  const slides = sortedEntries(pkg.entries.values(), /^ppt\/slides\/slide\d+\.xml$/i).slice(
    0,
...
async function extractWorkbookNames(
  pkg: ZipPackage,
  collector: TextCollector,
  signal: AbortSignal,
): Promise<void> {
...
async function detectPackageType(
  pkg: ZipPackage,
  signal: AbortSignal,
): Promise<OfficePackageType | null> {
