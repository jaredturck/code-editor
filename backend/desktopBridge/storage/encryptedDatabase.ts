...
import { decryptBuffer, decryptJson, decryptText, encryptBuffer, encryptJson, encryptText } from './encryption.js'
...
export async function initializeEncryptedDatabase({ databasePath, masterKey }: DatabaseOptions): Promise<void> {
...
function webSearchDisplayFromPayload(payload: Record<string, unknown>): Record<string, unknown> {
  ...
  const title = String(payload.title || query || 'Saved search')
    .trim()
    .slice(0, 160)
  const quick = payload.quick && typeof payload.quick === 'object' ? (payload.quick as Record<string, unknown>) : {}
  const detailed =
    payload.detailed && typeof payload.detailed === 'object' ? (payload.detailed as Record<string, unknown>) : {}
  return {
    query,
    title,
...
export async function listEncryptedWebSearchSessions(limit = 100): Promise<Record<string, unknown>[]> {
  const db = requireDatabase()
  const boundedLimit = Math.max(1, Math.min(MAX_WEB_SEARCH_HISTORY, Math.round(Number(limit) || 100)))
  const rows = await db.all<Record<string, unknown>>(
...
export async function createEncryptedWebSearchSession(value: unknown): Promise<Record<string, unknown>> {
  const db = requireDatabase()
...
export async function getEncryptedWebSearchSession(id: unknown): Promise<Record<string, unknown> | null> {
  const db = requireDatabase()
...
export async function upsertEncryptedWebSearchSession(id: unknown, value: unknown): Promise<Record<string, unknown>> {
  const db = requireDatabase()
  const sessionId = sanitizeWebSearchSessionId(id)
  if (!sessionId) throw new Error('web search session id required')
  const exists = await db.get<{ id: string }>('SELECT id FROM web_search_sessions WHERE id = ?', [sessionId])
  if (!exists) throw new Error('web search session not found')
  const payload = normalizeWebSearchPayload(value)
  const display = webSearchDisplayFromPayload(payload)
  const displayPayload = encryptJson(db.masterKey, 'web-search-display', sessionId, 'display', display)
  const sessionPayload = encryptJson(db.masterKey, 'web-search-payload', sessionId, 'payload', payload)
...
export async function duplicateEncryptedWebSearchSession(id: unknown): Promise<Record<string, unknown>> {
...
export async function deleteEncryptedWebSearchSession(id: unknown): Promise<number> {
  const sessionId = sanitizeWebSearchSessionId(id)
  if (!sessionId) throw new Error('web search session id required')
  const result = await requireDatabase().run('DELETE FROM web_search_sessions WHERE id = ?', [sessionId])
...
async function encryptedChatDisplay(row: Record<string, unknown>): Promise<Record<string, unknown>> {
  const db = requireDatabase()
  const id = String(row.id || '')
...
async function chatExists(id: string): Promise<boolean> {
  const row = await requireDatabase().get<{ id: string }>('SELECT id FROM chats WHERE id = ?', [id])
...
async function writeChatState(chatId: string, value: { memory: string; compacted: string }): Promise<void> {
  const db = requireDatabase()
...
export async function saveEncryptedArtifact({
  ...
  if (row) {
    ...
    if (chunk) {
      ...
      const encryptedChunk = encryptText(db.masterKey, 'artifact-chunk', `${id}:${chunkIndex}`, 'content', chunk)
      ...
      const encryptedMetadata = encryptJson(db.masterKey, 'artifact-metadata', id, 'metadata', metadata)
      await db.transaction(async () => {
        await db.run(
...
export async function listEncryptedArtifacts({ limit, chatId }: { limit?: unknown; chatId?: unknown } = {}): Promise<
  Record<string, unknown>[]
> {
  const db = requireDatabase()
  const cap = Number.isFinite(Number(limit)) ? Math.max(1, Math.min(MAX_ARTIFACTS_INDEXED, Number(limit))) : 200
  const normalizedChatId = sanitizeChatId(chatId)
  const rows = await db.all<Record<string, unknown>>(
    `SELECT id, metadata_ciphertext, metadata_nonce, metadata_tag,
            cipher_version AS metadata_cipher_version,
...
export async function writeEncryptedSubagentOutput(taskId: unknown, content: unknown): Promise<string> {
  const db = requireDatabase()
...
export async function deleteEncryptedUserSkill(profile: string, skillId: string): Promise<void> {
  await requireDatabase().run('DELETE FROM user_skills WHERE profile = ? AND skill_id = ?', [profile, skillId])
}

export type EncryptedFilesystemContentKind = 'directory' | 'text' | 'document' | 'pdf' | 'image' | 'video' | 'binary'
...
export function encodeFileEmbedding(values: ArrayLike<number>): Buffer {
  const length = Math.min(MAX_FILE_EMBEDDING_DIMENSIONS, Math.max(0, Math.floor(Number(values.length) || 0)))
...
function fileEmbeddingFloat32Values(buffer: Buffer, dimension: number): Float32Array {
  const boundedDimension = Math.max(0, Math.min(MAX_FILE_EMBEDDING_DIMENSIONS, Math.floor(Number(dimension) || 0)))
...
async function upsertEncryptedFileIndexMeta(db: IrisEncryptedDatabase, meta: Record<string, unknown>): Promise<void> {
  const now = Date.now()
  ...
  await db.run(
    `INSERT INTO file_index_meta(
     ...
     ON CONFLICT(id) DO UPDATE SET
       ...
       key_version = excluded.key_version,
       updated_at = excluded.updated_at`,
    [encrypted.ciphertext, encrypted.nonce, encrypted.tag, encrypted.cipherVersion, encrypted.keyVersion, now],
  )
}
...
export async function writeEncryptedFilesystemNodes(nodes: EncryptedFilesystemNodeInput[]): Promise<void> {
...
function filesystemNodeFromRow(db: IrisEncryptedDatabase, row: Record<string, unknown>): EncryptedFilesystemNodeRecord {
  const id = String(row.id || '')
...
export async function writeEncryptedFileEmbeddingProfile(profile: Record<string, unknown>): Promise<void> {
  const db = requireDatabase()
  const now = Date.now()
...
export function prepareEncryptedFileSemanticRecord(
  ...
  return {
    ...
    encryptedEmbedding: encryptBuffer(masterKey, 'file-semantic', fileId, 'embedding', embeddingBuffer),
  }
}

/** Builds the stable SQLite parameter list for one prepared semantic upsert. */
export function fileSemanticUpsertParameters(prepared: PreparedEncryptedFileSemanticRecord, now: number): unknown[] {
  return [
    prepared.fileId,
...
export async function writeEncryptedFileSemantics(records: EncryptedFileSemanticInput[]): Promise<void> {
  ...
  const parameterSets = preparedRecords.map((prepared) => fileSemanticUpsertParameters(prepared, now))
...
export async function writeEncryptedVideoFrameSemantics(records: EncryptedVideoFrameSemanticInput[]): Promise<void> {
  if (!records.length) return
  const db = requireDatabase()
  const now = Date.now()
  await db.transaction(async () => {
...
export async function readEncryptedVideoFrameSemantics(): Promise<EncryptedVideoFrameSemanticRecord[]> {
  const db = requireDatabase()
  const rows = await db.all<Record<string, unknown>>(
...
export async function countEncryptedFileConceptSources(
  ...
  return {
    vectorCount: Math.max(0, Number(imageRow?.vector_count || 0)) + Math.max(0, Number(videoRow?.vector_count || 0)),
    fileCount: Math.max(0, Number(imageRow?.file_count || 0)) + Math.max(0, Number(videoRow?.file_count || 0)),
  }
}
...
export async function readEncryptedFileConceptVectorPage(
  ...
  return rows.map((row) => {
    ...
    return {
      sourceSemanticId,
      fileId: String(row.file_id || ''),
      ...(options.source === 'video' ? { timestampMs: Math.max(0, Number(row.timestamp_ms) || 0) } : {}),
...
export async function writeEncryptedFileConcepts(concepts: EncryptedFileConceptInput[]): Promise<void> {
  ...
  await db.transaction(async () => {
    for (const concept of concepts) {
      ...
      const encryptedMetadata = encryptJson(db.masterKey, 'file-concept', conceptId, 'payload', concept.metadata)
      const encryptedCentroid = encryptBuffer(db.masterKey, 'file-concept', conceptId, 'centroid', centroidBuffer)
      await db.run(
...
export async function finalizeEncryptedFileConceptGeneration(generation: string): Promise<number> {
  ...
  await db.transaction(async () => {
    ...
    await db.run('DELETE FROM file_concepts WHERE generation = ? AND member_count < 2', [normalizedGeneration])
  })
  const row = await db.get<{ count: number }>('SELECT COUNT(*) AS count FROM file_concepts WHERE generation = ?', [
...
export async function readEncryptedFileConcepts(generation: string): Promise<EncryptedFileConceptRecord[]> {
  const normalizedGeneration = String(generation || '')
    .trim()
    .slice(0, 96)
...
export async function readEncryptedFileConceptMemberships(
  ...
  const normalizedIds = [...new Set(conceptIds.map((id) => String(id || '').trim()).filter(Boolean))]
...
export async function deleteEncryptedFileConceptGenerationsExcept(activeGeneration: string): Promise<void> {
...
export async function deleteEncryptedFileSemantics(fileIds: string[]): Promise<void> {
  ...
  await db.transaction(async () => {
    ...
    for (let start = 0; start < uniqueIds.length; start += 400) {
      const batch = uniqueIds.slice(start, start + 400)
      const placeholders = batch.map(() => '?').join(', ')
      await db.run(`DELETE FROM file_concept_memberships WHERE file_id IN (${placeholders})`, batch)
    }
...
function launcherEmbeddingValues(buffer: Buffer, dimension: number): number[] {
  const boundedDimension = Math.max(0, Math.min(MAX_LAUNCHER_EMBEDDING_DIMENSIONS, Math.floor(Number(dimension) || 0)))
  if (!boundedDimension || buffer.length !== boundedDimension * 4) {
...
export async function saveEncryptedLauncherIndex(
  ...
  await db.transaction(async () => {
    ...
    for (const application of boundedApplications) {
      ...
      const encryptedMetadata = encryptJson(db.masterKey, 'launcher-application', id, 'metadata', application.metadata)
      const encryptedEmbedding = encryptBuffer(db.masterKey, 'launcher-application', id, 'embedding', embeddingBuffer)
      await db.run(
...
export async function readEncryptedLauncherApplications(): Promise<EncryptedLauncherApplicationRecord[]> {
  const db = requireDatabase()
  const rows = await db.all<Record<string, unknown>>(
    `SELECT id,
            metadata_ciphertext, metadata_nonce, metadata_tag,
...
export async function purgeExpiredEncryptedState(): Promise<void> {
  await requireDatabase().run('DELETE FROM subagent_outputs WHERE expires_at IS NOT NULL AND expires_at < ?', [
    Date.now(),
  ])
}
