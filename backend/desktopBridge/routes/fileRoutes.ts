...
} from '../services/fileIndexSourceService.js';
import {
  browseDirectory,
...
async function resolveFileManagerExisting(
  inputPath: unknown,
  baseDir: string,
  enabled: boolean,
): Promise<string> {
...
async function resolveFileManagerDirectory(
  inputPath: unknown,
  baseDir: string,
  enabled: boolean,
): Promise<string> {
...
async function resolveFileManagerWritable(
  inputPath: unknown,
  baseDir: string,
  enabled: boolean,
): Promise<string> {
...
export async function handleFilesystemReadRoutes(
  ...
  if (pathname === '/api/local/fs/browse' && req.method === 'POST') {
    ...
    const result = await browseDirectory(
      currentPath,
      fileManager
        ? await fileManagerRootForPath(currentPath, baseDir)
        : await resolveDirectoryWithinRoot('.', baseDir),
    );
  ...
  if (pathname === '/api/local/fs/thumbnail' && req.method === 'POST') {
    ...
    const thumbnail = await createFileThumbnail(
      targetPath,
      Number(body.width) || 240,
      Number(body.height) || 240,
    );
  ...
  if (pathname === '/api/local/fs/find' && req.method === 'POST') {
    ...
    const result = await findFiles(resolvedRoot.rootPath, {
      ...
      maxResults: Number.isFinite(Number(body.maxResults))
...
export async function handleFileIndexRoutes(
  ...
  if (pathname === '/api/local/fs/semantic/concepts' && req.method === 'POST') {
    requireBridgePermission(securityContext, 'fileRead');
    const body = await readJsonBody(req);
    const groups = await searchFileSemanticConcepts(
      body.query,
      body.groupLimit,
      body.filesPerGroup,
    );
...
export async function handleFilesystemWriteRoutes(
  ...
  if (pathname === '/api/local/fs/write' && req.method === 'POST') {
    ...
    const body = await readJsonBody(req);
    const targetPath = await resolveFileManagerWritable(
      body.path,
      baseDir,
      body.fileManager === true,
    );
