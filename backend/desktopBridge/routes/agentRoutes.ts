...
export async function handleAgentRoutes(
  ...
  if (pathname === '/api/local/agent/task/post' && req.method === 'POST') {
    try {
      ...
      assertBelowLimit(totalQueuedTasks(agentTaskQueue), WORKLOAD_LIMITS.agentQueueGlobal, 'Global agent queue')
  ...
  if (pathname.startsWith('/api/local/agent/stream/') && req.method === 'GET') {
    try {
      ...
      assertBelowLimit(totalSseClients(agentSSEClients), WORKLOAD_LIMITS.agentSseGlobal, 'Global agent SSE clients')
      res.writeHead(200, {
