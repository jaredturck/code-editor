...
export function createDuckDuckGoSearchWindow(): DuckDuckGoSearchWindowService {
  ...
  const emit = (
    ...
    event: DuckDuckGoBrowserProgressEvent,
  ): void => {
  ...
  const performSearch = async (
    ...
    try {
      ...
      await raceWithAbort(
        Promise.race([
          ...
          timeoutAfter<void>(
            ...
            'DuckDuckGo browser navigation timed out.',
          ),
        ]),
        request.signal,
        cancelBrowser,
      ...
      const response: DuckDuckGoBrowserSearchResponse = {
        results,
        relatedQueries: Array.isArray(extracted.relatedQueries)
          ? extracted.relatedQueries.slice(0, 8)
          : [],
