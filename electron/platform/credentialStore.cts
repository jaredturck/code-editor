...
function createCredentialStore({
  ...
  if (!app || !safeStorage)
    throw new Error('Credential store requires Electron app and safeStorage');
  ...
  function status(): CredentialStoreStatus {
    ...
    if (
      platform === 'linux' &&
      typeof electronSafeStorage.getSelectedStorageBackend === 'function'
    ) {
  ...
  function assertAvailable(): CredentialStoreStatus {
    ...
    if (!current.available) {
      const error: CodedError = new Error(
        `Secure credential storage unavailable: ${current.reason}`,
      );
  ...
  function readStore(): CredentialStoreData {
    ...
    try {
      ...
      if (fileStats.isSymbolicLink())
        throw new Error('Credential store file must not be a symlink');
...
function registerCredentialIpc({ ipcMain, BrowserWindow, store }: CredentialIpcOptions): void {
  ...
  ipcMain.on('iris:credential-list', (event) =>
    respond(event, () => ({ providers: store.list() })),
  );
