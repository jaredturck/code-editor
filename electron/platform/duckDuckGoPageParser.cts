 ...
 */
export function extractDuckDuckGoPage(maxResults: number, timeoutMs: number): Promise<ExtractedDuckDuckGoPage> {
  ...
  const collect = (): ExtractedDuckDuckGoPage => {
    const primaryCards = Array.from(
      document.querySelectorAll('[data-testid="mainline"] li[data-layout="organic"] article[data-testid="result"]'),
    )
    const fallbackCards = Array.from(
      document.querySelectorAll('[data-testid="mainline"] article[data-testid="result"][data-nrn="result"]'),
    ...
    for (const card of cards) {
      ...
      const titleLink = card.querySelector<HTMLAnchorElement>('a[data-testid="result-title-a"][href]')
      const displayLink = card.querySelector<HTMLAnchorElement>('a[data-testid="result-extras-url-link"][href]')
