...
function modelIsInstalled(installed: Set<string>, model: string): boolean {
  ...
  return [...installed].some(
    (name) => name === expected || name.startsWith(`${expected}:`) || name.startsWith(`${expected}@`),
  )
}
...
async function pullOllamaModel(model: string): Promise<number> {
  ...
  if (!response.ok || payload.error) {
    throw new Error(`Ollama could not install ${model}: ${String(payload.error || `HTTP ${response.status}`)}`)
  }
