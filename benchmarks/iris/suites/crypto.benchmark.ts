...
function roundTripBuffer(context: CryptoContext, operations: number): Uint8Array {
  ...
  for (let index = 0; index < operations; index += 1) {
    ...
    output = decryptBuffer(context.masterKey, 'benchmark-buffer', `record-${index}`, 'payload', encrypted)
  }
  return output
}
...
export const cryptoBenchmarks: BenchmarkDefinition<any>[] = [
  {
    ...
    description: 'Derives the domain key, encrypts, authenticates, decrypts, and verifies a 1 KiB buffer.',
    iterations: 12,
    warmupIterations: 3,
  ...
  },
  {
    ...
    description: 'Measures encrypted artifact and larger state payloads with the production domain-key derivation.',
    iterations: 10,
    warmupIterations: 2,
  ...
  },
  {
    ...
    description: 'Measures large encrypted payload throughput without involving SQLite or filesystem I/O.',
    iterations: 8,
    warmupIterations: 2,
  ...
  },
  {
    ...
    description: 'Serializes, encrypts, decrypts, and parses a representative structured persistence payload.',
    iterations: 12,
    warmupIterations: 3,
