...
const mocks = vi.hoisted(() => ({
  ...
  onDesktopAgentStatus: vi.fn<
    (listener: (status: { running: boolean; thinking: string }) => void) => () => void
  >(() => () => {}),
...
describe('OrbContext', () => {
  ...
  it('persists changed settings but ignores identical updates', () => {
    ...
    expect(mocks.writeOrbSettings).toHaveBeenCalledWith(
      expect.objectContaining({ orb_size: 'large' }),
    );
  ...
  it('persists contextual permission grants after synchronizing the desktop bridge', async () => {
    ...
    expect(mocks.writeOrbSettings).toHaveBeenCalledWith(
      expect.objectContaining({ permissions_file_read: true }),
    );
  });
  ...
  it('deduplicates clipboard history', () => {
    ...
    expect(result.current.clipboardHistory.filter((item) => item === 'npm run dev')).toHaveLength(
      1,
    );
  });
