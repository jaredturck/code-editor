...
const { loadOrCreateStorageKey, removeLegacyRendererStorage } =
  require('../../electron/storageKeyStore.cjs') as {
...
describe('Electron application storage key', () => {
  ...
  it('does not replace a missing key for an existing encrypted database', async () => {
    ...
    await execSql(
      first.databasePath,
      'CREATE TABLE encrypted_store(key TEXT PRIMARY KEY); DELETE FROM storage_keys;',
    );
