...
const {
  ...
  createCollapsedLauncherShape: (orbBounds: ShapeRectangle, width: number, height: number) => ShapeRectangle[]
...
describe('launcher window shape geometry', () => {
  it('keeps the expanded launcher on one fixed native canvas', () => {
    expect(createExpandedLauncherShape(500, 500)).toEqual([{ x: 0, y: 0, width: 500, height: 500 }])
  })
  ...
  it('reduces a tucked collapsed launcher to the exact six-pixel recovery corner', () => {
    expect(createCollapsedLauncherShape({ x: -66, y: 494, width: 72, height: 72 }, 500, 500)).toEqual([
      { x: 0, y: 494, width: 6, height: 6 },
    ])
  })
