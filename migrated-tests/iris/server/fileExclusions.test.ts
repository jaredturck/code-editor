...
describe('file exclusions', () => {
  ...
  it.each(['Documents', 'Pictures', 'Projects', 'source', 'holiday-builds'])(
    'keeps ordinary directory %s',
    (name) => {
  ...
  it('detects excluded segments beneath a filesystem root', () => {
    ...
    expect(
      pathContainsExcludedDirectory(
        root,
        path.join(root, 'Projects', 'app', 'node_modules', 'library', 'index.js'),
      ),
    ).toBe(true);
    expect(pathContainsExcludedDirectory(root, path.join(root, 'Documents', 'notes.txt'))).toBe(
      false,
    );
  });
});
