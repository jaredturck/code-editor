...
import { resolveAndValidateRemoteUrl, safeRemoteRequestBuffer } from '../../server/desktopBridge/shared/networkSecurity'
import {
...
describe('network security boundary', () => {
  ...
  it('permits loopback only when the caller selects the loopback policy', async () => {
    ...
    expect(result.addresses.every((entry) => ['127.0.0.1', '::1'].includes(entry.address))).toBe(true)
  })
...
describe('provider proxy policy', () => {
  it('allows only the canonical hosted-provider API destination and path', () => {
    const allowed = createProviderProxyRequestPolicy('https://api.openai.com/v1/chat/completions', 'openai')
    expect(allowed.providerId).toBe('openai')

    expect(() => createProviderProxyRequestPolicy('https://example.com/v1/chat/completions', 'openai')).toThrow(
      'does not allow destination',
    )
    expect(() => createProviderProxyRequestPolicy('https://api.openai.com/admin', 'openai')).toThrow(
      'does not allow this API path',
    )
  })

  it('allows DeepSeek model discovery and chat requests only on its official host', () => {
    expect(createProviderProxyRequestPolicy('https://api.deepseek.com/v1/models', 'deepseek').providerId).toBe(
      'deepseek',
    )
    expect(
      createProviderProxyRequestPolicy('https://api.deepseek.com/v1/chat/completions', 'deepseek').providerId,
    ).toBe('deepseek')
    expect(() => createProviderProxyRequestPolicy('https://example.com/v1/chat/completions', 'deepseek')).toThrow(
      'does not allow destination',
    )
  })
