...
describe('encrypted storage primitives', () => {
  it('round-trips text, JSON, and binary payloads', () => {
    ...
    const binary = encryptBuffer(key, 'artifact-chunk', 'artifact-1:0', 'content', Buffer.from([0, 1, 2, 255]))
    ...
    expect(decryptBuffer(key, 'artifact-chunk', 'artifact-1:0', 'content', binary)).toEqual(Buffer.from([0, 1, 2, 255]))
  })
  ...
  it('rejects the wrong key and modified ciphertext or authentication tag', () => {
    ...
    expect(() => decryptText(randomBytes(32), 'chat-message', 'chat-1:1', 'payload', encrypted)).toThrow()
    ...
    expect(() => decryptText(key, 'chat-message', 'chat-1:1', 'payload', changedCiphertext)).toThrow()
  ...
  it('writes new payloads under the clean IRIS key version 1 and round-trips them', () => {
    ...
    expect(decryptText(key, 'chat-message', 'chat-1:1', 'payload', payload)).toBe('fresh IRIS secret')
  })
})
