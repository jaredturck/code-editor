...
} from '../../server/desktopBridge/services/launcherService';
...
describe('launcher service', () => {
  it('prefers desktop candidates and omits capabilities that cannot be resolved', async () => {
    ...
    expect(result.applications.find((item) => item.capability === 'terminal')?.displayName).toBe(
      'Konsole',
    );
    expect(result.applications.find((item) => item.capability === 'code_editor')?.displayName).toBe(
      'Sublime Text',
    );
  ...
  it('prefers the registered default browser over an installed fallback browser', async () => {
    ...
    await createExecutable(
      bin,
      'xdg-settings',
      'if [ "$1" = "get" ]; then echo custom-browser.desktop; fi',
    );
    await fs.writeFile(
      ...
      [
        ...
        'Type=Application',
        'Name=Custom Browser',
        `Exec=${customBrowser} --profile iris %u`,
      ].join('\n'),
    );
  ...
  it.skipIf(process.platform === 'win32')(
    'starts and stops only the managed development process group',
    async () => {
      ...
      await fs.writeFile(
        ...
        JSON.stringify({ name: 'demo', scripts: { dev: 'demo' } }),
      );
