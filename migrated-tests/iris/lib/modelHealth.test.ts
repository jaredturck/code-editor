...
describe('modelHealth', () => {
  ...
  it('returns null when no healthy distinct model is configured', () => {
    ...
    expect(
      pickFailoverModel(solo as never, { provider: 'anthropic', model: 'opus', keyId: '1' }),
    ).toBeNull();
  });
  ...
  it('resolves the failover policy (off / limited-N / exhaust + legacy + default)', () => {
    ...
    expect(
      resolveFailoverPolicy({ agent_failover_mode: 'limited', agent_failover_attempts: 3 }),
    ).toEqual({
      enabled: true,
      maxAttempts: 3,
    });
    expect(
      resolveFailoverPolicy({ agent_failover_mode: 'exhaust' }).maxAttempts,
