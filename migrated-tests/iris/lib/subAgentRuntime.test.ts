...
describe('subAgentRuntime', () => {
  ...
  it('extracts JSON from fenced model output', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(openAIResponse('```json\n{"answer":"fenced"}\n```')),
    );
  ...
  it('rejects forbidden tools and allows the model to recover', async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        openAIResponse('{"tool":"files.write","args":{"path":"a.txt","content":"x"}}'),
      )
  ...
  it('stops repeated local output without relying on a fixed step ceiling', async () => {
    ...
    const result = await executeSTP(
      makeTask({ budget: { maxSteps: 1, timeoutMs: 5000 } }),
      aiSettings,
    );
  ...
  it('executes explicit steps before requesting final output', async () => {
    const fetchMock = vi.fn((url: string) => {
      if (url === '/api/local/fs/read')
        return Promise.resolve(jsonResponse({ content: 'read result' }));
