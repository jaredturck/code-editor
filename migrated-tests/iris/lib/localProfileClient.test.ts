...
describe('localProfileClient', () => {
  ...
  it('preserves the existing local email/password-shaped UI behavior', async () => {
    await expect(localProfileClient.loginViaEmailPassword('user@example.test', 'password')).resolves.toEqual({
    ...
    await expect(localProfileClient.loginViaEmailPassword('', '')).rejects.toThrow('Email and password are required')
  })
  ...
  it('preserves registration, OTP, and reset UI flows locally', async () => {
    ...
    await expect(localProfileClient.resetPassword({ newPassword: '123' })).rejects.toThrow('at least 6 characters')
