...
describe('screen capture error messages', () => {
  ...
  it('distinguishes missing and unreadable sources', () => {
    expect(
      getScreenCaptureErrorMessage(Object.assign(new Error(), { name: 'NotFoundError' }), true),
    ).toContain('No shareable screen');
    expect(
      getScreenCaptureErrorMessage(Object.assign(new Error(), { name: 'NotReadableError' }), true),
