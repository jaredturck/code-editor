...
describe('toolCatalog', () => {
  ...
  it('gives delegated sub-agents the file-edit, research, and skill tools', () => {
    ...
    expect(names).toEqual(
      expect.arrayContaining(['files.patch', 'files.diff', 'web.fetch', 'launch.run', 'skills.load', 'skills.search']),
    )
    // Tier minimums: patch writes (STANDARD), skills are advisory (LOCKED → any tier).
    expect(getSubAgentMinTier('files.patch', PERMISSION_TIER.STANDARD)).toBe(PERMISSION_TIER.STANDARD)
    expect(getSubAgentMinTier('skills.load', PERMISSION_TIER.STANDARD)).toBe(PERMISSION_TIER.LOCKED)
  })
