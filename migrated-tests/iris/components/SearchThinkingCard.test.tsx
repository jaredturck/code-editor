...
describe('SearchThinkingCard', () => {
  it('shows streamed thinking live, collapses when the answer starts, and remains expandable', () => {
    const view = render(
      <SearchThinkingCard content="Reviewing **source evidence**" active answerStarted={false} />,
    );

    expect(screen.getByText('source evidence')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /model thinking/i })).toHaveAttribute(
      'aria-expanded',
      'true',
    );

    view.rerender(
      <SearchThinkingCard content="Reviewing **source evidence**" active answerStarted />,
    );
